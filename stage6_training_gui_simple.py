...
try:
    # 尝试导入1312转换器（文件名是1312_replay_converter.py）
...
class Stage6TrainingGUI:
    ...
    def create_widgets(self):
        ...
        concept_text = """从"预测卡牌的AI" → "赢得游戏的AI"
• ✅ 策略原因学习：理解"为什么这样选择"
• ✅ 胜率导向损失：学习"什么有效"
• ✅ 动态阈值调整：根据局面自适应
• ✅ 概率校准：提高预测准确性
• ✅ 综合评估：多维度验证效果"""
        ...
        self.epochs_var = tk.StringVar(value="80")  # 减少默认轮数，避免训练过久
        ...
        self.model_path_var = tk.StringVar(value="models/bc_model_stage6_simple.pth")
        ...
        monitor_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=5)
        ...
        self.reason_vars = {}
        for i, reason_type in enumerate(self.reason_types[:12]):  # 显示前12个
        ...
        # 文本形式的趋势图
        trend_frame = ttk.LabelFrame(right_frame, text="训练趋势图表", padding="5")
        trend_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        self.trend_text = scrolledtext.ScrolledText(
            trend_frame, height=20, width=60, wrap=tk.WORD,
            font=("Consolas", 8), bg="#f8f8f8"
        )
        self.trend_text.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.trend_text.insert(tk.END, "等待训练开始...\n")
    ...
    def update_trend_chart(self):
        ...
        if self.training_metrics['action_accuracy']:
            chart_text += f"动作准确率: {self.training_metrics['action_accuracy'][-1]:.2f}% "
            chart_text += self.create_mini_bar(self.training_metrics['action_accuracy'][-1], 100) + "\n"

        if self.training_metrics['strategy_accuracy']:
            chart_text += f"策略准确率: {self.training_metrics['strategy_accuracy'][-1]:.2f}% "
            chart_text += self.create_mini_bar(self.training_metrics['strategy_accuracy'][-1], 100) + "\n"
        ...
        if self.training_metrics['strategy_understanding']:
            understanding = self.training_metrics['strategy_understanding'][-1]
            chart_text += f"策略理解率: {understanding:.2f}% "
            chart_text += self.create_mini_bar(understanding, 100) + "\n"

        if self.training_metrics['reason_accuracy']:
            reason_acc = self.training_metrics['reason_accuracy'][-1]
            chart_text += f"原因学习准确率: {reason_acc:.2f}% "
            chart_text += self.create_mini_bar(reason_acc, 100) + "\n"
        ...
        if self.training_metrics['win_rate_loss']:
            loss = min(self.training_metrics['win_rate_loss'])  # 显示最小损失
            chart_text += ".4f"
            chart_text += "\n"
        ...
        # 训练进度
        chart_text += "⏱️ 训练进度:\n"
        progress = len(epochs) / max(1, int(self.epochs_var.get())) * 100
        chart_text += f"完成度: {progress:.1f}% "
        chart_text += self.create_mini_bar(progress, 100) + "\n"

        # 清空并更新
        self.trend_text.delete(1.0, tk.END)
        self.trend_text.insert(tk.END, chart_text)

    def create_mini_bar(self, value, max_value, width=20):
        """创建迷你进度条"""
        if max_value == 0:
            return ""
        ...
        try:
            ...
            if "Epoch" in output_line and "/" in output_line:
                ...
                if epoch_part.isdigit():
                    ...
                    if epoch not in self.training_metrics['epochs']:
                        ...
                        self.metrics_vars['epoch'].set(str(epoch))
            ...
            if "Action Accuracy - Exact:" in output_line:
                ...
                if "Card:" in output_line:
                    ...
                    self.metrics_vars['action_acc'].set(".2f")
            ...
            if "Strategy Accuracy - Overall:" in output_line:
                ...
                self.metrics_vars['strategy_acc'].set(".2f")
            ...
            if "Understanding Rate:" in output_line:
                ...
                self.metrics_vars['strategy_understanding'].set(".2f")
            ...
            if "Win Rate Loss:" in output_line:
                ...
                self.metrics_vars['win_rate_loss'].set(".4f")
    ...
    def start_stage6_training(self):
        ...
        # 检查是否有1312格式的数据需要转换
        if Replay1312Converter is not None:
            self.log("🔍 检查1312格式数据...")
            ...
            for json_file in json_files:
                ...
                try:
                    # 检查是否是1312格式（缺少game_info或all_players_hands）
