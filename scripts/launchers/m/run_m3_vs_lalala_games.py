...
log_file = log_dir / f"m3_vs_lalala_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("m3_vs_lalala")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="M3 vs lalala 批跑（BatchExecutor，队胜率 KPI 入口）",
    )
    parser.add_argument(
        "--games",
        type=int,
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("=" * 60)
    logger.info("M3 vs lalala - %d 局对战", games)
    logger.info("=" * 60)
    ...
    server_exe = get_server_exe(project_root)
    client_scripts = [str(project_root / s) for s in M3_CLIENT_SCRIPTS]
    for s in client_scripts:
        if not Path(s).exists():
            raise SystemExit(f"错误: 客户端脚本不存在: {s}")
    ...
    executor = BatchExecutor(
        ...
        diagnose_only=False,
        state_file=str(project_root / "m3_vs_lalala_state.json"),
        score_file=str(project_root / "m3_vs_lalala_scores.json"),
