...
try {
    ...
    Write-Host "? 已导航到项目根目录" -ForegroundColor Green
} catch {
    Write-Host "? 导航失败: $_" -ForegroundColor Red
...
Write-Host "   ? 服务器启动完成，等待客户端连接..." -ForegroundColor Green
Write-Host ""
...
foreach ($client in $ClientConfigs) {
    ...
    if (-not (Test-Path $clientPath)) {
        Write-Host "   ? 客户端文件不存在: $clientPath" -ForegroundColor Red
        continue
    }
    ...
    try {
        ...
        if ($clientProc) {
            ...
            Write-Host "      ? 客户端 $($client.Name) 已启动 (PID: $($clientProc.Id))" -ForegroundColor Green
        } else {
            Write-Host "      ? 客户端 $($client.Name) 启动失败" -ForegroundColor Red
        }
    } catch {
        Write-Host "      ? 客户端 $($client.Name) 启动失败: $_" -ForegroundColor Red
    }
...
try {
    ...
    if ($serverWindow -and $serverWindow.MainWindowHandle -ne [IntPtr]::Zero) {
        ...
        Write-Host "   ? 服务器窗口已激活" -ForegroundColor Green
    }
} catch {
...
if ($activatedCount -gt 0) {
    Write-Host "   ? 已激活 $activatedCount 个客户端窗口" -ForegroundColor Green
}
