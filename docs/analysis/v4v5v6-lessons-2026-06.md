...
- 全历史文档（`历次训练效果汇总.md` 1806 行 + `阶段0~8-*.md` + `方向评估与完整提升方案.md` 948 行）从未出现过"对 lalala 胜率 X%"
- V5 强化学习部分明确写"**评估次数=0，无法判断胜率**"
- **V5 早期甚至跑不起来**——`YF_V5_ANALYSIS.md` 直指根因：lalala 路径 `D:\NYGD\lalala` 根本不存在
...
**BC 预训练（从 10 样本 → 13409 样本）**：
- 阶段 1（10 样本）：准确率 59.41% — 过拟合
- 阶段 2（252 样本）：完全匹配率 23.81% — 预测过少 76.2%
- 阶段 3（13409 样本）：完全匹配率 1.53% → **最终 59.67%**（50 epoch + 加权 BCE + 缩放）

**强化学习（自对弈）**：
- 第一次：平均奖励 -1234.52
- 第二次：平均奖励 -1405.62（**比第一次差**）
- 第三次：+1316.35（**比第一次好 5.5 倍**）
- **评估次数：0**（明确写"无法判断胜率"）
...
| 教训 | 证据 | V7 应对 |
|------|------|--------|
| **从未测过对战胜率** | 历次报告全聚焦 loss/准确率；RL 部分评估次数=0 | V7-007 队胜率硬指标；GUA-039b 30 局 vs lalala 评估 |
| **训练指标好 ≠ 实战胜率高** | 完全匹配率 59.67% vs 胜率未知（很可能 < 30%） | V7 升格后 GUA-045 显式 guard；不准以"模型自信度高"为优化目标 |
...
| **组件堆砌失控** | V5 stage5 = pattern+opponent+dynamic+RL 四件套集成 | V7 升格：V7 自有空间单一路径；禁止 import M1/M2 训练代码 |
...
|------|------------|------|---------|
| `docs/training/历次训练效果汇总.md` | `6fd7b2b` | 1806 | V4/V5 训练参数 + 准确率演变；RL 评估次数=0 |
| `docs/training/方向评估与完整提升方案.md` | `6fd7b2b` | 948 | 阶段 0~8 实施计划；潜在问题分析；业界方法对比 |
| `docs/training/阶段5-高级策略学习.md` | `6fd7b2b` | 1395 | V5 stage5 设计（pattern+opponent+dynamic） |
| `YF_V5_ANALYSIS.md`（根目录） | `590531d` | 212 | **V5 跑不起来根因：lalala 路径错误** |
