...
3. `docs/analysis/agent-sessions/P0_IMPLEMENTATION_COMPLETE_20260528.md`（P0 实施清单）
4. `docs/analysis/agent-sessions/decision_patterns_sample.md`（最近 30 局决策模式抽样）
...
- 决策模式提取（M1/M2/M3 自适应）：
  ```
  python scripts/analyze_decisions.py --top-n 50 --md docs/analysis/agent-sessions/decisions_<date>.md
  ```

## 当前实测数据（2026-05-28 10:07 跑完批量 NUM_GAMES=30 → 20 局）

完整报告：`docs/analysis/agent-sessions/batch30_p0_trigger_vs_winrate_20260528.md`
...
- 见 `docs/analysis/agent-sessions/batch30_p0_trigger_vs_winrate_20260528.md`

**B. ~~排查 yf1/yf2 P0 触发不对称~~ ✅ 已排除（20 局后对称）**

**C. 推进 P1（强化学习 / 对手建模）**
- 见 `docs/analysis/agent-sessions/05-root-cause-analysis.md`

**D. M1 PASS 率仍 ~49% + 0 胜场，调主动出牌策略（最优先）**
- 重点看 `docs/archive/skill/出炸弹要领.txt` 和 `docs/guandan-brain/M2_OPTIMIZATION.md` "极端被动" 节
