...
| 2026-07-01 | **GUA-108/GUA-110/GUA-111** | **组牌候选桥接 + Q1 两手冲刺优先整牌 + Q1 同型通道归属剪枝后净盘批跑**。 | `python scripts/launchers/v7/run_v7_vs_lalala_games.py --games 3` | 3 | **0/3（0.0%）** | **38** | `completed_games=3/3`，`victoryNum=[0,3,0,3]`，同队镜像位校验通过。副胜 **8/38（21.1%）**；V7 级牌分布：2:5, 3:3, 4:3, 5:6, 6:3, 7:1, 8:3, 9:2, T:1, J:2, Q:2, K:2, A:5；末级：**≤5 17副 / J-K 6副 / A 5副**。说明残局止血后，V7 已能零星赢副，但仍无法把结构优势转成**局胜**。日志：`logs/v7_vs_lalala_20260701_175224.log`。 |
...
| 2026-07-28 | GUA-097-9game-BASELINE | [BASELINE 9局] GUA-091 关闭 | `V7_ENABLE_STAGE2_DISPATCH=0 run_v7_vs_lalala_games.py --games 9` | 9 | 7/9 (77.78%) | 338 | GUA-097 9局 baseline（stage_2 无 GUA-091 dispatch） |
| 2026-07-28 | GUA-097-9game-ENABLE | [ENABLE 9局] 启用 GUA-091 | `V7_ENABLE_STAGE2_DISPATCH=1 run_v7_vs_lalala_games.py --games 9` | 9 | 7/9 (77.78%) | 236 | GUA-097 9局 enable（GUA-091 中局 dispatch，覆盖 38.7% stage_2 步）；**结论：中性，不伤不补** |
...
| 2026-07-06 | GUA-116/117 | 主攻 P1–4 `stage_main_attack_lead` + 助攻 Layer0 B1–B6 `assist_layer0_guard`（commit `c2ed211`） | `python scripts/launchers/v7/run_v7_vs_lalala_games.py --games 3` | 3 | 0/3 (0.0%) | 78 | 净盘 WF-04；队胜 0/3；**副胜 14/39=35.9%**（环比 2026-07-01 21.1% **+14.8pp**）；vn=[0,3,0,3]；末级 A:5 副仍全输局；R-G080-4 零退化；L2: `logs/v7_vs_lalala_20260706_084309.log` + `yf1_v7_20260706_084341.log` + `yf2_v7_20260706_084348.log` |
| 2026-07-06 | GUA-123 | 敌方炸弹冲刺须反炸 + core-filter 豁免 + GUA-124 latent 诊断 | `python scripts/launchers/v7/run_v7_vs_lalala_games.py --games 3` | 3 | 0/3 (0.0%) | 84 | 净盘 post GUA-123；队胜 0/3；副胜 **11/42=26.2%**（环比 GUA-116/117 批 35.9% **−9.7pp**）；vn=[0,3,0,3]；末级 A:5；R-G080-4 零退化；GUA-124 警告 36 条；残局扫描 1 条；L2: `logs/v7_vs_lalala_20260706_222434.log` + `yf1_v7_20260706_222506.log` + `yf2_v7_20260706_222513.log` |
| 2026-07-10 | (本机观测) | 无权重 fallback：`bc_model_v3.pth` 缺失 → 规则引擎 | `python scripts/launchers/v7/run_v7_vs_lalala_games.py --games 3` | 3 | **1/3 (33.3%)** | 104 | **规则栈时代首个破零局胜**（06-06 后近月首次队胜；非 NN 挂载）。WF-04；队胜 **1/3**；副胜 **20/52=38.5%**（环比 GUA-123 26.2% **+12.3pp**）；vn=[1,2,1,2] P1 通过；末级 ≤5:20 / J-K:7 / A:7；模型使用率 **0%**；R-G080-4 **零退化**；L2: `logs/v7_vs_lalala_20260710_100635.log` + `yf1_v7_20260710_100708.log` + `yf2_v7_20260710_100712.log` |
| 2026-07-10 | GUA-110/116/135 续作后 | 整牌型领出 + 真已头游/冲刺能力止血后观测（净盘后本日累计 4×3） | `run_v7_vs_lalala_games.py --games 3`×2 + `--games 6` | **12** | **1/12 (8.3%)** | **170** | `analyze_v7_rounds --all`：4 会话 vn=`[0,3]×3` + `[1,2]`；副胜 **52/170=30.6%**；末级 2:22 / ≤5:60 / J-K:33 / A:22。末会话 **1/3**（vn=[1,2,1,2]，副胜 23/55=41.8%）。scores 仅记末次 `--games 6`→1/5。R-G080-4 **零退化**；残局扫描 4×`recommended_filtered_to_pass_only`。L2: `v7_vs_lalala_20260710_142317.log`（主）+ `141453`/`142108`；`yf1/yf2_v7_20260710_142512/142519`。`vn_source=gameResult` |
