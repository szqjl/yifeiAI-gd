...
| GUA-048 | open | P2 | observation, infra | v7, m1, m3, batch | **batch_executor 日志 dump 延迟 73s（stdout 缓冲/PIPE 满）** | 2026-06-07 V7 批跑（`v7_vs_lalala_20260607_135528.log`）实测：服务器端 actIndex 出牌时间 `13:56:35` 在主日志 dump 出来时已是 `13:57:48`，**dump 延迟 73s**。yf1_v7/2_v7 的**独立日志**时间戳（13:56:35）才是真实出牌时间；**主日志**（v7_vs_lalala_*.log）的 `[HH:MM:SS]` 来自 `batch_executor` 子进程 PIPE 缓冲 + 落盘周期。**根因嫌疑**：`batch_executor/restart_manager.py` 用 `subprocess.Popen(stdout=PIPE)` + 行缓冲读取，未 `flush()`；Windows 下默认 4KB 行缓冲+子进程独立进程组，dump 周期可能与 `process_group=True` 调度有关。**影响**：V7-007 队胜率 KPI 观测**实时性受损**——主日志时间戳不可信，需用 yf1/yf2 独立日志或 `lalala_adapter` 端事件时间戳对照；不修复则 KPI 统计（决策时间分布、卡顿检测）会**系统性偏慢**。**不在范围**：M3/M1 批跑主日志 dump 行为（保留 GUA-046 观察）。 | `batch_executor/restart_manager.py`；`batch_executor/executor.py`；`logs/v7_vs_lalala_*.log` | **登记 2026-06-07**；P2 升格依据：拉低 V7-007 KPI 实时性观测；**关单条件**：① 主日志时间戳与 yf1/yf2 独立日志时间戳偏差 ≤ 5s；② pytest 覆盖 dump 延迟检测（`tests/test_batch_dump_latency.py`）；③ 不破坏 M3/M1 批跑；**相关**：GUA-047（误判关单的根因）；GUA-046（副间 ~10s 观察，仍 open） |
