...
| GUA-046 | open | P3 | observation, infra | v7, m1, m3, batch | **副间 ~10s 停顿（服务器内在结算/发牌间隔）** | V7 批跑日志显示每副牌决策集中在 **1 秒内**打完，之后服务器进入 **~10s** 无消息空窗期（episodeOver → 算分 → 贡还 → 发牌 → 下一副 act）。非 V7 客户端问题——M1/M3 同样经历该间隔，只是 GUI 模式下不如 cmd 窗口显眼。**对比验证**：`RUN_V7_VS_LALALA.bat` cmd 窗口停顿 10s 光标不动；`batch_executor_gui_m3.py` 同服同客户端架构，日志框缓冲不觉。**根因**：`guandan_offline_v1006.exe` 内置固定副间结算流程，客户端无法控制。 | 无（服务器行为） | **订正（2026-06-07）**：本条原定性"副间 ~10s 停顿"经 GUA-047 复查后被覆盖；GUA-047 实测为**同副牌内 4 席全停 20s**，根因待定位（**非**本条所说的"副间结算间隔"）。本条仅保留副间/开局 18s 等待等**已确认服务器行为**的观察，不再承担同副内停顿的归因 |
| GUA-047 | open | **P1** | observation, infra | v7, m1, m3, batch | **同副牌内 4 席全停 ~20s（根因待定位，非副间结算）** | 2026-06-07 V7 批跑 cmd 窗口观察（`docs/analysis/批跑cmd窗口观察.md`）：**4 席（yf1 / yf2 / lalala-client3 / lalala-client4）在游戏进行中（已收牌若干轮）突然全部停止输出 20 秒**，之后继续打牌直至 3 局结束。**实测**：yf2 窗口 12:29:55 起密集出牌至 12:29:55 末条后停顿，lalala client3 窗口在连续 Bomb/单牌流水后同样停顿，**两窗尾部均人工标注"暂停 20 秒"**——CEO 现场确认**4 席全停而非单席卡顿**。**误分类史**：GUA-046 原把"4 席全停 20s"定性为"副间 ~10s 服务器结算间隔"（**与本条现象不匹配**），已订正 GUA-046 备注。**根因（待定位）**：嫌疑是 `guandan_offline_v1006.exe` 单步慢操作 / 内部 GC / 算牌耗时，但**未定位**——是 exe 内部还是平台网络层、是否在 lalala 出 Bomb/特殊牌型时必现、是否在 M3/M1 批跑同样发生，均待证。**与 GUA-044 区别**：GUA-044 是**首局开局 30-60s 卡顿**（已闭单，门闩机制修），GUA-047 是**同副内中段停顿**（现象、时段、性质均不同）。**影响**：4 席全停 20s ≈ 1/3 局时间浪费在空窗，**直接拉低 V7-007 队胜率采样效率**；如不修复，9 局/批的有效决策时间远低于预期，KPI 统计可能失真。 | `docs/analysis/批跑cmd窗口观察.md`；`logs/v7_vs_lalala_*.log`（需 4 席并发时间戳对照） | **登记 2026-06-07**；P1 升格依据：拉低 V7-007 采样效率 + 根因未定位；**不在范围**：副间结算间隔（保留 GUA-046 观察）；**下一步**：① 拉 4 席并发日志做时间线对照（v7_vs_lalala_*.log 同时段）；② 复现实验在 M3/M1 批跑同样观察是否复现；③ 若锁定 exe 内部 → 反馈南邮平台；④ 若是网络层 → 排查 `guandan_offline_v1006.exe` → 客户端中间环节 |
