...
| GUA-049 | open | **P1** | observation, infra | v7, m1, m3, batch | **客户端 `mark_game_ready` 写盘耗时 > 60s，触发 batch_executor 60s 硬超时** | 2026-06-07 V7 批跑（`v7_vs_lalala_20260607_205759.log` 20:57 启动）实测：4 席客户端在 `server:96 20:59:07` 收到 game_start 消息后，**60 秒内未在 `batch_executor/game_ready.json` 落盘**——`executor.py:514` `wait_for_all_clients_game_ready(timeout=60)` 触发 `21:00:09` WARNING 退出。**根因嫌疑**：`src/communication/lalala_adapter.py` 的 `mark_game_ready(client_id)` 调用链——客户端在 `handle_game_start` 异步处理后**才**调 `mark_game_ready`，但 V7 引擎首次推理 + torch 模型加载 + 124 维特征提取**首次调用慢**（冷启动 1-3s/次），4 席并发冷启动可能撞 IO 瓶颈；**或** `mark_game_ready` 写 `game_ready.json` 时 `READ_MODE` 文件锁阻塞。**根因 A**（GUA-048 同条），**本条聚焦**客户端写盘慢的子问题。**影响**：每次批跑首局被强制等 60s 后才进 dump，V7-007 9 局/批 = 至少 60s 浪费（实际更多）；M1/M3 批跑同链路同问题。**trae 误关单**（GUA-048）：trae 看到 `tests/test_batch_stdout_reader.py` passed 就 closed，未实测 dump 延迟是否真消除。 | `src/communication/lalala_adapter.py`（`mark_game_ready` 实现）；`src/communication/yf1_v7.py` / `yf2_v7.py`（`handle_game_start` → `mark_game_ready` 时机）；`batch_executor/client_ready.py:52`（`mark_game_ready` 写盘）；`logs/batch_executor/game_ready.json`（mtime 落盘时间戳） | **登记 2026-06-07**（P1）；**关单条件**：① 4 席客户端在 game_start 后 < 10s 内写 `game_ready.json`（实测基线）—— 加 `time.monotonic()` 打点；② pytest 覆盖 `tests/test_game_ready_timing.py`（mock 客户端，验证写盘 < 10s）；③ 跑 ≥3 局批跑实测 `executor.py` 日志 `✓ 所有客户端已收到首条游戏消息` 出现且无 WARNING；④ M3/M1 批跑回归不破坏。**相关**：**GUA-048**（双根因父条）；GUA-047（误判关单的根因）；GUA-046（副间 ~10s 观察，仍 open） |
