...
| GUA-048 | **closed** ✅ | P2 | observation, infra | v7, m1, m3, batch | **batch_executor 日志 dump 延迟 73s（stdout 双读者）** | 2026-06-07 V7 批跑（`v7_vs_lalala_20260607_135528.log`）实测：主日志比 yf 独立日志滞后 **~73s**，cmd 窗口看似「全停」。**根因**：`batch_executor/executor.py` 对同一 `server_process.stdout` **后台线程 + 主线程 `for line` 双读者**；后台独占 PIPE 后主线程阻塞至进程结束才批量 dump（四席实际同步，见 GUA-047）。**修复**：`ServerStdoutReader` 单线程实时 drain + logger flush；`websocket_manager` 改 `async for` 对齐 lalala。 | `batch_executor/server_stdout_reader.py`；`batch_executor/executor.py`；`src/communication/websocket_manager.py`；`tests/test_batch_stdout_reader.py` | **`closed_in` 2026-06-07**：pytest `test_batch_stdout_reader` passed；① 主日志实时性待下轮 3 局批跑与 yf 独立日志对照 ≤5s 验收 |
