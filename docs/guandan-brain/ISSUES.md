...
| | **(2026-06-17 GUA-038 M3胜局重训+批跑验证)** | — | V-nn, v7 | v7 | **BC v2 M3胜局重训（action_dim 512→2048）** | **2026-06-17**：用 M3 批跑 366 局胜局（`game_records/`，100%胜率）重训 BC 模型 → `bc_dataset.py` 加载 **5555 样本** → 训练 1 epoch 即 early stopping（val_acc=35.19%）→ 模型保存 `models/v-nn/bc_model_v2.pth` → V7 引擎加载 → **12 局批跑验证**：V7 队胜率 **0/12（0%）**，副级 V7 赢 0/236（0%），lalala 全胜。日志 `logs/v7_vs_lalala_20260617_145158.log` | `scripts/train_bc_v7.py`、`src/v/nn/ultimate_win_rate_engine_v7.py`、`models/v-nn/bc_model_v2.pth` | **问题诊断**：① 训练样本不足（仅 1 epoch，patience=5 太早停止）；② M3 规则引擎决策模式可能不适配 V7 NN 架构；③ 需增加训练轮次/调整学习率/或改用 V7 自身高质量对局数据。**累计胜率 1/33（3.0%）**，距 GUA-039b 门槛 30% 差距巨大。 |
...
| GUA-050 | open | **P0** | V-nn, v7 | v7 | **局面信念向量 8 维**（扩展 GUA-037a） | V7 需从 `P(action|state)` 升级为 `P(action|state, belief)`。在 037a 静态特征后叠加 8 维信念：my_strength/partner_strength/opponent_pressure/level_progress/trump_ready/bomb_count/opponent_bomb_risk/last_card_meaning。解决套路文档 §1「标准 RL 无信念中间变量」的根本问题。**2026-06-17 副级分析验证**：`scripts/tools/analyze_v7_round_levels.py` 对 06-17 批跑 102 副分析确认：V7 赢 8 副（7.8%），lalala 赢 94 副（92.2%）；V7 达 A 级 8 副；但 V7 在赢的副里均**先跑光**（order[0]=V7位置），说明**跑牌决策正确**，问题在于 lalala 在相邻副里**先双上 A** 抢走整局——正是「谁先双上」全局节奏问题，信念向量需捕捉「队友已升几级 / 对手逼近 A」的博弈信号。 | `src/v/nn/features/static_features.py`、`src/decision/ultimate_win_rate_engine_v7.py` | **登记 2026-06-16**（P0，GUA-037a 扩展）；来源于 `docs/analysis/v7-re-eval-2026-06.md` §2.2；特征维数 124→132；风险：信念不收敛→降为 4 维核心。**完成定义**：信念向量在线推理 ≤1ms；pytest ≥6 case；V7 净盘 3 局观测（不作关单） |
...
| GUA-054 | open | **P0** | V-nn, v7, policy | v7 | **V7 组牌质量中间表示**（grouping_scanner 9 维，借鉴 v4 策略库经验） | v4 模型因"纯规则 vs 艺术的鸿沟"0 胜封停；V7 需将"组牌艺术"作为软信号注入而非硬编码规则。基于 **6 大组牌核心原则**（去单化/小火大轮次/减少轮次/炸弹平衡/逢人配留变化/牌型连贯）+ v4 策略库组牌优先级（StraightFlush>Bomb>Straight>...）→ V7-internal `grouping_scanner.py` 输出 9 维 grouping_score。解决 V7 副级胜率波动 0~13.8% 的"组牌艺术"缺失问题。**严格遵守 §7.4 升格硬约束**：V7 训练/推理代码禁止 `from src.m.m3 import ...` 任何形式；V7-internal 实现 | `src/v/nn/features/grouping_scanner.py`（新建）、`src/v/nn/features/memory_tracker.py`（追加 9 维）、`src/decision/ultimate_win_rate_engine_v7.py`（`_extract_features` 对齐 229 维）、`src/v/nn/training/bc_dataset.py`（`EFFECTIVE_FEATURE_DIM` 220→229） | **登记 2026-06-17**（P0，v4 经验教训 + 6 大组牌原则落地）；来源于 `docs/architecture/掼蛋AI客户端架构方案.md` v4 封停复盘 + `docs/guandan-brain/6大组牌核心原则` 记忆 + ITERATIONS 2026-06-17 V7 累计 1/33（3.0%）。**完成定义**：① 9 维特征（bomb_count/sequence_count/triple_count/pair_count/single_count/longest_run/wild_flexibility/round_efficiency/hand_strength）；② pytest ≥10 case（每个组牌原则至少 1 case + 性能 <1ms）；③ 接入 MT 24→33 维（共 229 维，特征利用率 220→229/512=44.7%）；④ V7 净盘 3 局批跑验收，副级胜率不下降（**不回归** GUA-050/052 接入时的 REGRESSION）；⑤ **不引入 if-else 硬规则**（v4 教训：6 因素权重 25+20+20+15+10+10 静态权重不可行）。**相关**：GUA-050/051/052/053（已实施的 V7 方案增补）；GUA-038（M3 胜局重训）；GUA-055（同步立项） |
| GUA-055 | open | **P0** | V-nn, v7, policy | v7 | **V7 动作空间二阶段过滤**（借鉴 v4 ActionSpaceOptimizer，纯函数式） | v4 `ActionSpaceOptimizer` 用启发式 Top-K 解决 5000+ 动作空间，但因 Top-K 后精细评估仍是规则而撞 lalala SOTA 上限。V7 改造：**启发式 Top-K 100（纯函数式，不调参）** → V7 NN 精细评估剩余候选。解决 V7 220 维→2048 action_head 直接 argmax 时的计算/评估质量问题。**v4 经验**：启发式可继承，规则权重不能继承 | `src/v/nn/features/action_space_filter.py`（新建，纯函数式 Top-K 100）、`src/decision/ultimate_win_rate_engine_v7.py`（`decide()` 二阶段：filter→NN 评估→argmax）、`tests/test_v7_action_space_filter.py`（≥6 case） | **登记 2026-06-17**（P0，v4 经验继承）；来源于 `docs/architecture/掼蛋AI客户端架构方案.md` §3.3.6 ActionSpaceOptimizer。**完成定义**：① Top-K 启发式（**不调参，纯函数**）：按"牌型大小 + 手牌结构影响 + 压制能力"打分，保留 Top-100；② pytest ≥6 case（边界：动作数 <100/=100/>100/全 PASS/单动作）；③ V7 引擎 `decide()` 二阶段接入；④ V7 净盘 3 局批跑验收；⑤ 启发式 + NN 总耗时 <0.8s（决策超时上限）；⑥ **不作硬规则**——Top-K 启发式只筛不排序，最终排序交 V7 NN。**相关**：GUA-054（同时立项，先 GUA-054 后 GUA-055） |
| GUA-056 | open | P1 | V-nn, v7, training | v7 | **V7 双上节奏 reward**（v4 缺失项补足，扩展 GUA-051） | v4 6 因素评估中"配合 20%"是单步概念，缺"队友也跑光"的**双上节奏全局概念**。V7 副级 8/58 仍局级 0/3，正是"双上节奏"弱（ITERATIONS 2026-06-17 §"V7 在赢的副里均先跑光，但 lalala 在相邻副里连庄双上"）。扩展 GUA-051 9 种 reward 加 1 种 `team_double_up_reward`：队友前 2 名跑光时 +1.0 | `src/v/nn/training/reward.py`（追加 `team_double_up_reward` + `RewardAccumulator` 集成）、`tests/test_v7_reward_gua056.py`（≥4 case） | **登记 2026-06-17**（P1，v4 缺失项补足）；来源于 `docs/architecture/掼蛋AI客户端架构方案.md` §3.3.4 CooperationStrategy 单步设计 + ITERATIONS 2026-06-17 副级分析。**完成定义**：① `team_double_up_reward` 实现：my_finish_pos 与 partner_finish_pos 综合判断；② 触发条件：`(partner_finish_pos ≤ 2 and my_finish_pos ≤ 2) → +1.0`，否则 0；③ pytest ≥4 case（双上成功/单上/均未上/边界）；④ **待 BC → RL 切换后启动**（当前 BC 阶段不直接生效）；⑤ V7 净盘 3 局观测（RL 启动后）；⑥ **v4v5v6-lessons 红线约束**：`docs/analysis/v4v5v6-lessons-2026-06.md` §0/§4.1/§6 明确"每条 V7 GUA 实施前必问：这会让 V7-007 队胜率上升吗？答否重新设计"——GUA-056 是"加 1 种 reward"对实战胜率影响未实测，**必须先 GUA-039b 30 局 vs lalala 评估拿到对战基线**，再实施 GUA-056；⑦ **不可独立关单**：GUA-056 实施后须在 v4v5v6-lessons 增补"实施后 vs lalala 30 局对账"表，验证队胜率相对 GUA-039b 基线确实上升才可关单（避免 V4-V5"训练侧虚高、无对战 KPI"覆辙）。**相关**：GUA-051（已 closed，9 种 reward）；GUA-054/055（同步立项）；GUA-039b（前置对战基线，open）；v4v5v6-lessons-2026-06.md §0/§4.1/§6 红线 |
| GUA-057 | open | **P1** | V-nn, v7, policy | v7 | **V7 记牌模块**（4×27 维 = 108 维剩余牌概率分布，LSTM/Transformer encoder-decoder） | 出牌历史 → 各家剩余牌分布概率（4 家 × 27 张 one-hot = 108 维）。V7 当前 GUA-052 24 维 state_vector 仅"已出牌计数"级别，缺"未出牌概率"推断——V7 必须建模"对手还剩什么牌"，否则组合出牌质量（炸弹判定/顺子可行性/接风可行性）无法评估。**严格遵守 §7.4 升格硬约束**：V7-internal 实现，禁止 `from src.m.m3 import ...` 任何形式。**前置依赖**（经验教训）：需 GUA-039b 30 局 vs lalala 基线 + GUA-054 实施验证后才启动 | `src/v/nn/features/card_counting_network.py`（新建，LSTM/Transformer encoder + 108 维 decoder）、`src/v/nn/training/bc_dataset.py`（追加出牌历史序列字段）、`src/decision/ultimate_win_rate_engine_v7.py`（特征接入，220→328 维） | **登记 2026-06-17**（P1，模块化训练 Week 2 立项占位）；来源于 `docs/guandan-brain/掼蛋AI自我进化-随机应变套路.md` §套路三 Memory/长程情境记忆 + §套路七 模块化分阶段训练 Week 2。**完成定义**：① LSTM/Transformer encoder-decoder 输入出牌历史序列，输出 4×27 维剩余牌概率分布；② 训练数据：game_records 完整对局（≥500 局）；③ 推断准确率>80%（对比真实剩余牌 restCards）；④ 性能 <2ms/步；⑤ pytest ≥8 case（推断准确性 + 性能 + 边界）；⑥ V7 净盘 3 局批跑验证（**仅观测不关单**）。**前置依赖**：① GUA-039b 30 局 vs lalala 基线完成；② GUA-054 实施且 9 维 grouping_score 接入验证；③ GUA-052 24 维 state_vector 复用为 encoder 输入特征。**相关**：GUA-050/051/052/053/054/055/056（V7 方案增补）；掼蛋AI自我进化-随机应变套路.md §套路三 + §套路七 Week 2 |
| GUA-058 | open | **P1** | V-nn, v7, policy | v7 | **V7 策略模块**（4 分类：进攻/防守/观望/保对家） | 组牌+记牌 → 策略分类（进攻/防守/观望/保对家）。V7 当前 GUA-050 8 维 state_belief 仅"局面信念向量"是连续特征，缺**离散策略选择**——V7 必须把"动态贝叶斯局面判断"显式建模为策略分类（套路文档 §套路一），再用条件 NN 学动作概率。**严格遵守 §7.4 升格硬约束**：V7-internal 实现。**前置依赖**（经验教训）：需 GUA-057 记牌模块完成 + GUA-039b 基线 + GUA-054 实施验证后才启动 | `src/v/nn/features/strategy_network.py`（新建，4 分类 classifier）、`src/v/nn/training/strategy_dataset.py`（M3 胜局策略自动标注）、`src/decision/ultimate_win_rate_engine_v7.py`（策略特征接入） | **登记 2026-06-17**（P1，模块化训练 Week 3 立项占位）；来源于 `docs/guandan-brain/掼蛋AI自我进化-随机应变套路.md` §套路一 局面信念建模 + §套路七 模块化分阶段训练 Week 3。**完成定义**：① 4 分类 NN：输入=组牌结果+记牌结果+局面信念，输出=4 类概率分布（进攻/防守/观望/保对家）；② 训练数据：M3 胜局自动标注（用 M3 当局实际打法反推策略分类），≥300 局；③ 分类准确率>85%（5-fold cross validation）；④ 性能 <1ms/步；⑤ pytest ≥6 case（4 类平衡 + 边界 + 性能）；⑥ V7 净盘 3 局批跑验证（**仅观测不关单**）。**前置依赖**：① GUA-057 记牌模块 closed；② GUA-039b 30 局 vs lalala 基线完成；③ GUA-054 实施且 9 维 grouping_score 接入验证；④ GUA-050 8 维 state_belief 复用为策略 NN 输入。**相关**：GUA-050/051/052/053/054/055/056/057（V7 方案增补）；掼蛋AI自我进化-随机应变套路.md §套路一 + §套路七 Week 3 |
| GUA-059 | open | **P0** | V-nn, v7, training | v7 | **GUA-038 BC v2 退化根因定位**（副胜率 11.8%→0%，训练→实战脱钩） | v7-win-rate-history.md 06-17 三次批跑曲线明确显示退化：① 第 2 次 12 局副胜率 **11.8%**（val_acc=35.85%）；② 第 3 次 12 局副胜率 **0%**（val_acc=35.19%，1 epoch 早停）。**根因未明**——训练侧指标与实战侧表现完全脱钩，疑似 GUA-038 BC v2 重训（action_dim 512→2048）**引入了 V7 NN 架构不兼容的偏差**。所有下游 GUA（039b/056/057/058）链式依赖基线，必须先恢复基线。**严格遵守 v4v5v6-lessons §0 红线**："训练指标漂亮没用，必须有 ≥30 局实测 KPI" | `scripts/train_bc_v7.py`、`src/v/nn/training/bc_dataset.py`、`models/v-nn/bc_model_v2.pth`（退化）、`models/v-nn/bc_model_v1.pth`（基线，如已覆盖需 git 找回）、`game_records/`（366 局 M3 胜局）、`logs/v7_vs_lalala_20260617_130258.log`（11.8%）、`logs/v7_vs_lalala_20260617_145158.log`（0%） | **登记 2026-06-17**（P0，所有 P1 GUA 的硬前置）；来源于 `docs/guandan-brain/v7-win-rate-history.md` 06-17 退化曲线 + `v4v5v6-lessons-2026-06.md` §0 红线。**完成定义**：① **代码层**：比对 v1 vs v2 模型架构差异（action_dim 512→2048 是否导致输出空间爆炸 / loss 不收敛）；② **数据层**：比对 v1 vs v2 训练样本（bc_dataset.py 5555 样本 vs 前次样本量，1 epoch 早停原因）；③ **决策层**：在同 1 副 game_records 输入下，v1 vs v2 决策动作分布是否一致（同输入不同输出 → 训练不稳定）；④ **批跑层**：v1 vs v2 在同 1 局（restCards 序列可重现）下的实战副胜率；⑤ **根因锁定**（5 候选）：a) 训练数据采样偏差（bc_dataset 5555 样本分布不均）；b) 模型架构 action_dim 2048 爆炸（512→2048 引入稀疏）；c) M3 决策模式与 V7 NN 编码空间不匹配（M3 启发式 vs NN 端到端）；d) 1 epoch 早停权重未收敛（val_acc=35.19% → 远低于 v1 82.57%）；e) 训练-推理 dtype/设备/输入归一化不一致；⑥ **修复 + 验证**：根据根因制定修复 → 12 局批跑验证副胜率 ≥ 11.8%（恢复 GUA-038 前一次水平）；⑦ **关单硬条件**：≥30 局 vs lalala 队胜率 + 副胜率均不下降（v4v5v6-lessons §0 红线）。**相关**：v7-win-rate-history.md 06-17；GUA-038（已实施，触发退化）；GUA-039b（硬前置，30 局 3.0%）；GUA-056/057/058（链式依赖）；v4v5v6-lessons-2026-06.md §0/§4.1/§6 |
| GUA-060 | open | **P0** | V-nn, v7, training | v7 | **V7 BC val_acc 锁死 36.46% 根因 + C 方向调参**（argmax collapse 到 top1） | 2026-06-17 GUA-059 阶段 1 修复后重训：5541 样本（action_index<512 过滤 14 个）→ val_acc=36.46% 6 epoch 锁死 ≈ top1 action 占比 35.75%（action_index=0）；A 诊断：唯一 action 166/512（67.6% 未用），top5 累计 72.19%，罕见 action（<5 样本）121/166（72.9%），归一化熵 0.489。**根因**：模型 argmax collapse 到 top1（信息论上限）；B-α 调参（label_smoothing=0.1 + lr 5e-4 + patience=10）失败——train_loss 9.12e7 爆炸 + val_acc 仍锁死 36.46%（**val_acc 锁死与 loss 解耦**），证实"调参方向无法解锁 argmax 锁死"。**必须**改目标分布（class_weight）或换 loss（focal loss） | `scripts/train_bc_v7.py`（B-α 失败改动待回退）、`src/v/nn/training/bc_trainer.py`（`masked_cross_entropy` 第 102 行 label_smoothing=0.1 hard-code 失败改动待回退）、`models/v-nn/bc_model_v2.pth`（36.46% 最新）、`models/v-nn/bc_model_v2_GUA060_20260617_36pct.pth`（640KB 备份）、`models/v-nn/bc_model_v2_PRE_RETRAIN_20260617.pth`（1.19MB 旧备份 2048 维退化）、`tmp_action_dist_diag.py`（A 诊断 ad-hoc 脚本） | **登记 2026-06-17**（P0，GUA-059 子方向，**待交接给同事继续做 C 方向调参**）；来源于 GUA-059 阶段 2 重训 val_acc 锁死 + A 诊断 + B-α 失败。**完成定义**：① **C-α（推荐起步）**：回退 label_smoothing→0 + 加 class_weight balance（top1 0.5x / 长尾 1.5x 反比加权）→ 重训 val_acc > 36.46%；② **C-β（升级方案）**：用 focal loss（γ=2.0）替代 CE → 强制关注低频 action；③ **C-γ（备选）**：强正则（dropout 0.2→0.5 + weight_decay 1e-4→1e-3）→ 抑制 top1 过拟合；④ **清理失败改动**：`bc_trainer.py:102` 的 label_smoothing=0.1 hard-code 回退（PyTorch CE + masked logits 数值不稳定） + `train_bc_v7.py:42-50` 改回 lr=1e-3 + patience=5；⑤ **关单硬条件**：val_acc > 50% + 12 局批跑副胜率 ≥ 11.8%（GUA-038 前次水平）+ ≥30 局 vs lalala 队胜率不下降（v4v5v6-lessons §0 红线）。**当前文件状态**：`src/train/ultimate_win_rate_training.py:45` action_head 32→512（GUA-059 阶段 1 修复 ✅ 保留）；`bc_dataset.py:37` TARGET_ACTION_DIM=512（✅ 保留）；`src/v/nn/ultimate_win_rate_engine_v7.py` Guard 接入 ✅ + grouping_score 9 维串联 ✅（GUA-059 阶段 1 修复全部保留，**勿回退**）；`bc_trainer.py:102` label_smoothing=0.1（B-α 失败改动 ⚠️ 待回退）；`train_bc_v7.py:42-50` lr=5e-4 + patience=10（B-α 失败改动 ⚠️ 待回退）；`bc_model_v2.pth` val_acc=36.46%（最新）；备份 `bc_model_v2_GUA060_20260617_36pct.pth` 640KB + 旧备份 `bc_model_v2_PRE_RETRAIN_20260617.pth` 1.19MB（2048 维退化）均保留。**相关**：GUA-059（open，本条为子方向）；GUA-038（BC 基线，触发退化）；v4v5v6-lessons-2026-06.md §0 红线；`tmp_action_dist_diag.py`（A 诊断证据） |
