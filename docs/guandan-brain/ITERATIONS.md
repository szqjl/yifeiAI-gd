...
> **当前活跃**：**GUA-062 同花顺候选枚举 + NO_STRAIGHTS 双变体** — _detect_straight_flushes 全候选枚举（低牌组SF优先）+ SF_FIRST 多方案 + NO_STRAIGHTS trips 灵活参与顺子；**GUA-062 A→2 包接+多pass去单化** — 3 处包接 + SF_FIRST 多pass loop；**GUA-062 v2 权重调优** — 手数0.3+去单化0.2+回收0.1；**GUA-062 bc_model_v3 12局批跑结论** — 队胜 0%、BC argmax collapse 无解；**GUA-054** — 动作空间过滤（优先）；**GUA-059** — 退化为观测
