...
| 2026-08-02 | 实验线（无 GUA） | 无代码变更。**卡死复现验证重跑**（净盘后 3 局；上批 `logs\v7dan_vs_danzero_20260802_150938.log` 遇 v1006 服务器 A 级 65 副长局后开新局偶发卡死、completed_games=0/3） | `python scripts/launchers/v7dan/run_v7dan_vs_danzero_games.py --games 3` | 3 | **3/3（100%）** | **62** | 本批 vn `[3,0,3,0]`（批末校验通过，scores.json `team_a_wins=3`）；副胜 **44/62（71.0%）**；末级分布 2:9/3:3/4:3/5:6/6:6/7:3/8:5/9:6/T:1/J:2/Q:7/K:2/A:9；Team A 位次 yf1 头游 44/62（71.0%）；Team B 达 A 副数 9；`restart_count=0`、`completed_games=3/3`、运行 6m28s；残局扫描异常 0；日志 `logs\v7dan_vs_danzero_20260802_155151.log`。**结论：上批卡死为 v1006 服务器偶发（A 级长局触发），本次复跑无卡死，KPI 无退化**（21 局累计 90.5% 维持）。 |
