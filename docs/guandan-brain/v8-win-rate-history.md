...
| 2026-08-24 | **GUA-234 + GUA-072** | 动态重组 P0–E + 规则记牌 M1–M5 + rule_memory 训练接入后阶段 F 护栏批跑 | `python scripts\launchers\v8\run_v8_vs_lalala_games.py --games 9` | 9 | **9/9（100%）** | 84 | 78.6%（66/84） | 36.9%（31/84） | ✅ scores.json 对齐；`restart_count=8`；R-G080-4=0；残局扫描 2×recommended_filtered_to_pass_only；末游率 yf1=13.1% yf2=26.2%；vs 基线 83.3% **护栏通过**；日志：`logs\v8_vs_lalala_20260824_160114.log` |
