...
|----------------------|--------|--------|------|
| 掼蛋AI客户端架构方案.md | `architecture/` | 有效·需对齐 | 模块架构总纲；需与 M/V、`src/m/`、`src/v/` 对照更新；**正文部分乱码待修复** |
| 掼蛋AI完整开发指南.md | `development/` | 有效·需对齐 | 开发全流程；frontmatter 含未渲染的 `datetime.now()` 模板；**部分乱码** |
| 掼蛋AI相关比赛汇总.md | `competition/` | **有效** | UTF-8 正常；平台与赛事索引（2026-04-21 更新） |
| 掼蛋AI知识应用框架.md | `knowledge/` | 有效·需对齐 | 知识应用设计；与 `docs/knowledge/` 结构对照；**部分乱码** |
...
| 一等奖代码优秀特点分析.md | `competition/` | 有效·需对齐 | lalala 一等奖分析；**正文乱码待修复** |
| 知识库格式化方案.md | `knowledge/` | 有效·需对齐 | 与 `knowledge/rules`、`skills` 目录对齐；**部分乱码** |
| cleanup_summary.md | `governance/archive/` | 有效·归档 | 2025-12-17 大文件清理记录 |
| DanZero+论文分析-架构借鉴建议.md | `analysis/` | **有效** | RL 论文借鉴，UTF-8 正常 |
| DEVELOPMENT_RULES.md | `development/` | 有效·需对齐 | 与根目录 `CLAUDE.md` 时间规则一致；**部分乱码** |
...
| REPLAY_TRAINING_GUIDE.md | `training/` | 有效·需对齐 | 回放训练；**部分乱码** |
...
| 主题 | 真源 | 备注 |
|------|------|------|
| 掼蛋基础知识 | `knowledge/guandan-basic-knowledge.md` | `claude-analysis/guandan-basic-knowledge.md` 为分析副本，非真源 |
...
3. 发现乱码文件：用 UTF-8 重新保存后在本表将「待修复」改为「有效」。
