...
## 当前问题
- ❌ Layer 1 (YF) 几乎总是返回有效动作
- ❌ Layer 2 (DecisionEngine) 很少执行
- ❌ Layer 3 (Knowledge) 从不执行
- ❌ 知识库设计为"增强层"但实现为"后备层"
...
- [ ] 修改 `_try_yf()` 返回候选动作列表而非单一动作
- [ ] 修改 `_try_decision_engine()` 返回候选动作列表
- [ ] 添加 `_generate_candidates()` 方法整合 Layer 1/2
- [ ] 添加 `_select_best()` 方法选择最优动作
...
**第一步**: 阶段 1 Task 1.1 - 添加关键规则层
**第二步**: 阶段 4 Task 4.1 - 创建关键规则模块
**第三步**: 阶段 1 Task 1.2 - 修改主流程
**第四步**: 阶段 3 Task 3.1 - 修改为增强模式
