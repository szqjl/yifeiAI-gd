...
class WebSocketManager:
    ...
    def __init__(self, user_info: str, use_local: bool = True,
        ...
        Args:
            ...
            platform: 平台类型 "v1006"（旧）或 "openguandan"（新）
            v8_role: V8 房间角色 "creator" 或 "joiner"（仅 openguandan）
            v8_round_count: V8 局数（仅 creator 有效）
        """
        self.user_info = user_info
        self.use_local = use_local
        self.platform = platform  # V8: 新平台标识
        self.v8_role = v8_role
        ...
        self._adapter = None  # OpenGuanDanAdapter 实例
    ...
    def _get_connection_uri(self) -> str:
        """获取连接URI（V8: openguandan 用端口 8181）"""
        if self.platform == "openguandan":
            if self.use_local:
                return "ws://127.0.0.1:8181"
            else:
                return self.ws_config.get("network_url_v8", "ws://[局域网IP]:8181")
        # 旧平台（v1006）
        if self.use_local:
    ...
    async def _do_room_handshake(self) -> None:
        """V8 OpenGuanDan: 连接后执行 CREATE_ROOM 或 JOIN_ROOM 握手"""
        import json as _json
        from pathlib import Path as _Path

        room_file = _Path(__file__).parent.parent.parent / "tmp" / ".v8_room_id"

        if self.v8_role == "creator":
            # yf1: 创建房间
            msg = self._adapter.create_room(self.v8_round_count)
            self.logger.info("[V8] 发送 CREATE_ROOM: %s", _json.dumps(msg, ensure_ascii=False))
            await self.websocket.send(_json.dumps(msg))
            resp_raw = await asyncio.wait_for(self.websocket.recv(), timeout=10)
            resp = _json.loads(resp_raw)
            self.logger.info("[V8] CREATE_ROOM 响应: %s", _json.dumps(resp, ensure_ascii=False))
            
            room_data = resp.get("data", {})
            room_id = room_data.get("roomId")
            if room_id is None:
                raise RuntimeError(f"CREATE_ROOM 未返回 roomId: {resp}")
            self.room_id = room_id
            self._adapter.room_id = room_id
            
            # 写入共享文件供其他客户端读取
            room_file.parent.mkdir(parents=True, exist_ok=True)
            room_file.write_text(str(room_id), encoding="utf-8")
            self.logger.info("[V8] roomId=%s 已写入 %s", room_id, room_file)
        else:
            # yf2 / lalala: 等待 roomId 然后加入
            import time as _time
            deadline = _time.monotonic() + 30
            while _time.monotonic() < deadline:
                if room_file.exists():
                    room_id = int(room_file.read_text(encoding="utf-8").strip())
                    self.room_id = room_id
                    self._adapter.room_id = room_id
                    self.logger.info("[V8] 读取 roomId=%s 来自 %s", room_id, room_file)
                    break
                await asyncio.sleep(0.5)
            else:
                raise RuntimeError(f"等待 roomId 超时 (30s)，文件 {room_file} 未就绪")
            
            msg = self._adapter.join_room(self.room_id)
            self.logger.info("[V8] 发送 JOIN_ROOM: %s", _json.dumps(msg, ensure_ascii=False))
            await self.websocket.send(_json.dumps(msg))
            resp_raw = await asyncio.wait_for(self.websocket.recv(), timeout=10)
            resp = _json.loads(resp_raw)
            self.logger.info("[V8] JOIN_ROOM 响应: %s", _json.dumps(resp, ensure_ascii=False))
    ...
    async def connect(self, max_retries: int = -1) -> bool:
        ...
        while self.should_reconnect and (max_retries < 0 or retry_count < max_retries):
            try:
                ...
                # V8: OpenGuanDan 房间握手
                if self.platform == "openguandan" and self._adapter:
                    await self._do_room_handshake()
