# -*- coding: utf-8 -*-
"""
yf1_v6_stage5 - YiFei AI V6 Stage5 Client (Player 0)
...
# 日志文件名：yf1_v6_YYYYMMDD_HHMMSS.log
...
class yf1_v6_Client:
    ...
    async def connect(self):
        ...
        try:
            # 使用 WebSocket 管理器连�?            self.logger.info(f"[yf1_v6] 等待连接延迟 {DELAY_BEFORE_CONNECT} 秒，确保第一个位�?..")
            time.sleep(DELAY_BEFORE_CONNECT)
            self.logger.info(f"[yf1_v6] 开始连�?ws://127.0.0.1:23456/game/yf1_v6")
            ...
            # 显示连接成功和期望位置信�?            print(f"[yf1_v6] 连接成功！期望位置：{self.player_id}号位（实际位置将在游戏开始时由服务器分配�?)
        ...
        try:
            ...
            # 分析动作列表，为所有动作生成策略评�?            for idx, action in enumerate(action_list):
                ...
                if is_active and action_type != "PASS":
                    ...
                    # 主动出牌时，单张减分（除非是级牌、大王等�?                    elif action_type in ["Single", "SINGLE"]:
                        if action_cards:
                            card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                            if card_rank not in ['2', 'B', 'R']:  # 不是级牌、大�?                                score_adjustment -= 20.0
                ...
                if is_active and action_type != "PASS":
                    ...
                    if action_type == "Single" or action_type == "SINGLE" and has_king_or_level:
                        # 有王/级牌保护，单张能回收，提升优先级至对子级�?                        score_adjustment += 80.0
                        strategy_reason = "主动出牌策略：有�?级牌保护，单张优先级提升至对子级�?
                    # 1. 对子优先级最高（+80分）
                ...
                if action_type == "BOMB" or "BOMB" in str(action_type).upper() or action_type == "Bomb" or action_type == "StraightFlush":
                    ...
                    # 如果没有"不炸"建议，再检查是否有"�?的建�?                    if not should_not_bomb:
                        for sugg in bomb_suggestions:
                            ...
                            if "�? in action_text and "不炸" not in action_text:
                                should_bomb = True
                                # 根据不同�?�?原因，给予不同的加分
                                if "改牌�? in action_text:
                                    score_adjustment += 60.0  # 改牌路是最重要的，大幅加分
                                elif "枪打第一�? in action_text:
                                    score_adjustment += 55.0  # 枪打第一顺，大幅加分
                                elif "控牌" in action_text:
                                    score_adjustment += 50.0  # 控牌，大幅加�?                                elif "追炸" in action_text:
                                    score_adjustment += 50.0  # 追炸，大幅加�?                                elif "冲刺" in action_text or "阻冲�? in action_text or "阻听�? in action_text:
                                    score_adjustment += 55.0  # 冲刺相关，大幅加�?                                elif "打上�? in action_text or "打下�? in action_text or "主攻" in action_text:
                                    score_adjustment += 45.0  # 打特定对手，加分
                                elif "及时" in action_text or "准确" in action_text or "价�? in action_text:
                                    score_adjustment += 50.0  # 及时、准确、价值，加分
                                elif "�?出对" in action_text or "�?" in action_text or "�?" in action_text or "�?" in action_text or "下家�?" in action_text:
                                    score_adjustment += 45.0  # 残局特定牌数，加�?                                elif "早炸大王" in action_text or "级牌�? in action_text:
                                    score_adjustment += 50.0  # 早炸大王、级牌对，加�?                                elif "先小后大" in action_text:
                                    # 先小后大，需要判断炸弹大小（简化处理）
                                    score_adjustment += 35.0  # 中等加分
                                else:
                                    score_adjustment += 40.0  # 一般炸的建�?                                strategy_reason = reason_text
                                break
                    
                    # 如果没有明确的建议，检查当前牌型是否值得�?                    if not should_bomb and not should_not_bomb:
                ...
                if game_phase == "endgame" or opponent_rest_cards <= 10:
                    ...
                    # 2. 报双.须打单诱其拆
                    elif "报双诱拆" in endgame_reason or "打单" in endgame_action or "打单（报双诱拆）" in endgame_action:
                        if action_type == "Single" or action_type == "SINGLE":
                            score_adjustment += 40.0
                            strategy_reason = "残局策略：报双打单诱�?
                    
                    # 3. 报单.只能打非单牌�?                    elif "报单打非�? in endgame_reason or "不打�? in endgame_action or "不打单（报单打非单）" in endgame_action:
                        if action_type == "Single" or action_type == "SINGLE":
                            score_adjustment -= 30.0
                    ...
                    # 4. 出小单（有王回收�?                    elif "出小单（有王回收�? in endgame_action:
                        if action_type == "Single" or action_type == "SINGLE":
                            if action_card_count == 1 and action_cards:
                                card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                if card_rank_str in ['3', '4', '5', '6', '7', '8', '9']:
                                    score_adjustment += 45.0
                                    strategy_reason = "残局策略：剩3张有王，先出小单再用王回收冲�?
                    
                    # 5. 出单倒着打：从大往小打
                    elif "倒着�? in endgame_action or "从大往�? in endgame_reason or "出单倒着打（从大往小）" in endgame_action:
                        if action_type == "Single" or action_type == "SINGLE":
                            if action_card_count == 1:
                                card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
    ...
    def handle_notification(self, data: dict):
        ...
        if is_beginning:
            ...
            if my_pos != self.player_id:
                print(f"[yf1_v6] 位置更新：期望位置{self.player_id} �?实际位置{my_pos}号位（服务器根据连接顺序分配�?)
                self.logger.info(f"Position updated: {self.player_id} -> {my_pos} (服务器根据连接顺序分�?")
                old_player_id = self.player_id
                self.player_id = my_pos
                
                # 重新初始化HybridDecisionEngineV6（使用新�?player_id�?                self.logger.info(f"Reinitializing HybridDecisionEngineV6 with player_id={my_pos}")
                config = {"performance_threshold": 1.0}
                self.decision_engine = HybridDecisionEngineV6(my_pos, config)
                self.logger.info(f"HybridDecisionEngineV6 reinitialized for position {my_pos}")
            else:
                # 位置与期望一致，也显示一�?                print(f"[yf1_v6] 我在{my_pos}号位（与期望位置一致）")
            ...
            # 增强兼容性：尝试更多可能的等级信息字�?            if self_rank == "?":
                # 尝试从其他可能的位置获取等级信息
...
async def main():
    ...
    client = yf1_v6_Client(player_id=0)
