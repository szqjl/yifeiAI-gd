# -*- coding: utf-8 -*-
"""
yf2_v7dan - YiFei AI V7 Client v7Dan (Player 2)
v7 vs DanZero 批跑骨架中的 v7 侧席位 2（队A次席）。
决策引擎与 yf2_v7 完全一致（UltimateWinRateEngineV7）；
仅席位标识改为 yf2_v7dan、牌谱写入独立目录 game_records_v7dan。
"""
import asyncio
import json
import sys
...
log_filename = log_dir / f"yf2_v7dan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class YF2_V7DAN_Client:
    ...
    def __init__(self, player_id=2, use_local_websocket=True):
        ...
        self.user_info = "yf2_v7dan"
        self.logger = logging.getLogger(f"yf2_v7dan")
        ...
        self.game_recorder = GameRecorder(player_id, "yf2_v7dan", record_dir=V7DAN_RECORD_DIR)
        ...
        self.logger.info(f"✓ yf2_v7dan initialized (Player {player_id})")
    ...
    async def connect(self):
        ...
        try:
            self.logger.info(f"[yf2_v7dan] 等待连接延迟 {DELAY_BEFORE_CONNECT} 秒，确保第三个位置...")
            time.sleep(DELAY_BEFORE_CONNECT)
            self.logger.info(f"[yf2_v7dan] 开始连接 ws://127.0.0.1:23456/game/yf2_v7dan")
            ...
            print(f"[yf2_v7dan] 连接成功！期望位置：{self.player_id}号位")
    ...
    def handle_game_start(self, data: dict):
        ...
        try:
            ...
            self.logger.info(
                "[座位排查] 来源=yf2_v7dan.handle_game_start, 原始myPos=%s, 原始playerPosition=%s, 同步后player_id=%s",
    ...
    async def handle_action_request(self, data: dict):
        ...
        try:
            ...
            self.logger.info(
                "[座位排查] 来源=yf2_v7dan.handle_action_request, 原始myPos=%s, 原始playerPosition=%s, 同步后player_id=%s",
            ...
            print(f"[yf2_v7dan] 选择动作: {act_index}")
