# -*- coding: utf-8 -*-
"""
yf2_v6_stage5 - YiFei AI V6 Stage5 Client (Player 2)
Enhanced version with Stage5 advanced AI capabilities
阶段5升级版本：集成策略模式识别、对手建模、动态策略调�?"""
import asyncio
import json
import sys
import logging
...
from decision.hybrid_decision_engine_V6 import HybridDecisionEngineV6
...
# 日志文件名：yf2_v6_YYYYMMDD_HHMMSS.log
log_filename = log_dir / f"yf2_v6_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class yf2_v6_Client:
    """
    YiFei AI V6 Client - Player 2
    Enhanced version with:
    - Improved RL integration (智能RL决策)
    - Enhanced knowledge base application (增强知识库应�?
    - Better decision fusion (更好的决策融�?
    """
    
    def __init__(self, player_id=2, use_local_websocket=True):
        self.player_id = player_id
        self.user_info = "yf2_v6"
        self.logger = logging.getLogger(f"yf2_v6")
        
        # 初始�?WebSocket 管理器（从配置文件读取设置）
        self.ws_manager = WebSocketManager(self.user_info, use_local=use_local_websocket)
        self.websocket = None  # 保持向后兼容
        
        # Initialize HybridDecisionEngineV6 (V6混合决策引擎)
        self.logger.info("🎯 Initializing HybridDecisionEngineV6")
        config = {"performance_threshold": 1.0}
        self.decision_engine = HybridDecisionEngineV6(player_id, config)
        
        # Initialize Dynamic Grouping Optimizer (动态组牌优化器)
        self.grouping_optimizer = DynamicGroupingOptimizer()
        
        # Initialize RL Engine (V6增强：更智能的RL集成)
        try:
        ...
        # V6特性：智能决策融合
        self.use_hybrid_decision = True  # 使用混合决策（RL + Knowledge + Rule-based�?        self.rl_weight = 0.4  # RL决策权重（提高，让训练出的模型真正发挥作用）
        self.knowledge_weight = 0.3  # 知识库权�?        self.rule_weight = 0.3  # 规则引擎权重（降低，给RL更多空间�?        
        ...
        # 维护连续PASS计数（用于special模式�?        self.pass_num = 0  # 全局连续PASS次数
        self.my_pass_num = 0  # 自己连续PASS次数
        
        # Initialize game recorder
        self.game_recorder = GameRecorder(player_id, "yf2_v6")
        
        self.logger.info(f"�?yf2_v6 initialized (Player {player_id})")
    ...
    async def connect(self):
        ...
        try:
            # 使用 WebSocket 管理器连�?            self.logger.info(f"[yf2_v6] 等待连接延迟 {DELAY_BEFORE_CONNECT} 秒，确保第三个位�?..")
            time.sleep(DELAY_BEFORE_CONNECT)
            self.logger.info(f"[yf2_v6] 开始连�?ws://127.0.0.1:23456/game/yf2_v6")
            connected = await self.ws_manager.connect()
            if not connected:
                self.logger.error("Failed to connect to server")
                return
            
            # 设置 websocket 引用以保持向后兼�?            self.websocket = self.ws_manager.websocket
            
            # 显示连接成功和期望位置信�?            print(f"[yf2_v6] 连接成功！期望位置：{self.player_id}号位（实际位置将在游戏开始时由服务器分配�?)
    ...
    async def handle_action_request(self, data: dict):
        """Handle action request from server (V6增强决策)"""
        self.decision_count += 1

        # 检查是否是贡牌或还牌阶�?        stage = data.get("stage", "")
        if stage == "tribute":
            self._handle_tribute_action(data)
        elif stage == "back":
            self._handle_back_action(data)
        else:
            # 普通play阶段，显示等级信息（类似client3/4�?            self_rank = data.get("selfRank", "?")
    ...
    def _extract_game_state(self, data: dict) -> dict:
        ...
        opponent_rest_cards = 27  # 默认�?        my_pos = data.get("myPos", self.player_id)
        ...
        # 判断是否为主动出�?        is_active = (cur_pos == -1 or greater_pos == -1)
        ...
        # 获取对手剩余牌数列表（用于判断残局�?        opponent_rest_cards_list = [27, 27, 27]
        if public_info and isinstance(public_info, list):
            for i, info in enumerate(public_info):
                if isinstance(info, dict):
                    rest = info.get("rest", 27)
    ...
    def _apply_strategy_suggestions(self, game_state: dict, action_list: list) -> list:
        ...
        应用策略建议，将策略建议转换为动作评�?        
        Returns:
            候选动作列�?[(action_index, score, source), ...]
        """
        candidates = []
        ...
        # 准备手牌组合信息（用于normal/special模式判断�?        # 获取牌值映�?        rank_map = {'3':3, '4':4, '5':5, '6':6, '7':7, '8':8, '9':9, 'T':10, 'J':11, 'Q':12, 'K':13, 'A':14, '2':15, 'B':16, 'R':17}
        ...
        # 获取当前牌型的最大值（用于残局判断�?        greater_action = game_state.get("greater_action", [])
        max_val = 0
        ...
        # 获取下家剩余牌数（用于残局判断�?        opponent_rest_cards_list = game_state.get("opponent_rest_cards_list", [27, 27, 27])
        numofnext = opponent_rest_cards_list[1] if len(opponent_rest_cards_list) > 1 else 27
        
        try:
            # 策略应用顺序（按优先级）�?            # 1. 组牌策略 - 优先考虑组牌效果（减少轮次、减少单牌）
            # 2. 牌力评估 - 评估当前牌力
            # 3. 单牌策略 - 根据牌力给出单牌建议
            # 4. 炸弹策略 - 根据牌力给出炸弹建议
            # 5. 残局策略 - 根据残局情况给出建议
            ...
            # 1. 组牌策略（优先：减少轮次、减少单牌）
            # 获取剩余牌数信息（用于红心配策略判断�?            my_rest_cards = len(hand_cards)
            ...
            # 判断是否是队友出牌（在调用grouping_strategy之前�?            my_pos = game_state.get("my_pos", self.player_id)
            teammate_pos = (my_pos + 2) % 4
            ...
            grouping_sugg = grouping_strategy(
                ...
                game_phase=game_phase,
                power=power,  # 使用实际计算的牌�?                cur_rank=game_state.get("cur_rank", "2"),  # 传递级牌信�?                my_rest_cards=my_rest_cards,  # 传递自己剩余牌�?                opponent_rest_cards=opponent_rest_cards,  # 传递对手剩余牌�?                is_teammate_action=is_teammate_action  # 传递是否是队友出牌
            )
            ...
            # 如果动态优化器建议调整，应用动态优化结�?            if dynamic_grouping_rec:
                ...
                if best_idx >= 0 and best_idx < len(grouping_sugg.get("suggestions", [])):
                    for sugg in grouping_sugg["suggestions"]:
                        if sugg["action_index"] == best_idx:
                            # 应用动态优化评分调�?                            sugg["score"] = dynamic_score
                            if dynamic_reason:
                ...
                # 记录动态优化信�?                self.logger.info(f"[动态组牌优化] 建议动作索引: {best_idx}, 评分: {dynamic_score:.2f}, 原因: {dynamic_reason}")
            ...
            # 3. 单牌策略（基于牌力评估结果）
            # 检查手牌中是否有王、级牌、大对子�?            has_king = any('B' in card or 'b' in card for card in hand_cards)  # �?            has_level_card = any(game_state["cur_rank"] in card for card in hand_cards)  # 级牌
            # 检查是否有Q以上的对�?            rank_map = {'3':3, '4':4, '5':5, '6':6, '7':7, '8':8, '9':9, 'T':10, 'J':11, 'Q':12, 'K':13, 'A':14, '2':15, 'B':16, 'R':17}
            rank_count = {}
            ...
            # 判断是否上家出单（顺上家�?            cur_pos = game_state.get("cur_pos", -1)
            greater_pos = game_state.get("greater_pos", -1)
            ...
            # 判断对手是否不接小单牌（简化：需要历史记录，这里暂时设为False�?            opponent_not_accept_small_single = False  # TODO: 需要从历史记录中判�?            
            ...
            # 判断是否双贡（简化：根据游戏阶段判断�?            is_double_tribute = (game_phase == "opening" and opponent_rest_cards >= 25)
            
            # 判断队友是否需要单牌（简化：需要从历史记录中判断）
            teammate_needs_single = False  # TODO: 需要从历史记录中判�?            
            # 判断对手是否需要单牌（简化：需要从历史记录中判断）
            opponent_needs_single = False  # TODO: 需要从历史记录中判�?            
            # 判断是否刚炸过（简化：需要从历史记录中判断）
            just_bombed = False  # TODO: 需要从历史记录中判�?            
            ...
            # 提取单张牌点列表（用于判断高�?中单/低单�?            single_card_ranks = []
            for card in hand_cards:
                if len(card) >= 2:
                    rank_str = card[1] if len(card) == 2 else card[1:2]
                    if rank_count.get(rank_map.get(rank_str, 0), 0) == 1:  # 是单�?                        single_card_ranks.append(rank_str)
            ...
            opponent_has_single = False  # TODO: 需要从历史记录中判�?            
            ...
            # 计算天然单张数量和是否有天然单张
            # 天然单张定义�?            # 1. 整幅手牌只有唯一的一�?            # 2. 级牌、小王、大王不要列入天然单�?            cur_rank = game_state.get("cur_rank", "2")
            
            # 筛选天然单张：排除级牌、小王、大�?            natural_singles = []
            for card in single_member:
                # 检查是否是级牌、小王、大�?                if card[-1] == cur_rank or card[-1] == 'B' or card[-1] == 'R':
                    continue
            ...
            # 4. 炸弹策略（基于牌力评估结果，应用完整知识体系�?            # 提取对手动作信息
            greater_action = game_state.get("greater_action", [])
            ...
            if greater_action and len(greater_action) > 0:
                ...
                # 计算牌点（简化：根据牌型判断�?                if opponent_action_type in ["Single", "SINGLE"]:
            ...
            # 判断是否为最后一�?            cur_pos = game_state.get("cur_pos", -1)
            greater_pos = game_state.get("greater_pos", -1)
            my_pos = game_state.get("my_pos", self.player_id)
            is_last_player = (cur_pos == 3) or (greater_pos != -1 and (greater_pos + 1) % 4 == my_pos)
            
            # 判断能否改变牌型（简化：如果对手出的是我方没有的牌型�?            can_change_card_type = opponent_action_type not in ["PASS", "none"] and opponent_action_type not in ["Single", "SINGLE", "Pair", "PAIR"]
            
            # 判断牌型是否明朗（简化：根据游戏阶段�?            card_type_clear = game_phase != "opening"
            ...
            bomb_sugg = bomb_strategy(
                ...
                has_clear_next_action=True,  # 简化：假设有明确出�?                card_type_clear=card_type_clear
            )
            
            # 5. 残局策略（基于牌力评估结果，集成单张技巧残局规则�?            # 获取对手剩余牌数列表（简化：需要从game_state中获取）
            opponent_rest_cards_list = game_state.get("opponent_rest_cards_list", [27, 27, 27])
            ...
            # 准备 sorted_cards �?bomb_info（需要从手牌组合中获取）
            # 由于 calculate_card_power 不返�?sorted_cards，我们需要从 grouping_strategy 或重新计�?            # 简化处理：使用空字典，bomb_selector 会处理这种情�?            sorted_cards_dict = {}
            bomb_info_dict = {}
            # 尝试�?grouping_sugg 中获取（如果有的话）
            # 如果没有，bomb_selector 会根�?action_list �?hand_cards 进行判断
            ...
            endgame_sugg = endgame_strategy(
                ...
                action_list=action_list,  # 传入动作列表，用于判断能否一手出�?                hand_cards=hand_cards,  # 传入手牌，用于判断能否一手出�?                sorted_cards=sorted_cards_dict,  # 传入已组合的手牌
                bomb_info=bomb_info_dict,  # 传入炸弹信息
                rank_card=rank_card  # 传入级牌
            )
            
            # 3. 根据策略建议调整动作评分
            # 分析动作列表，为所有动作生成策略评�?            for idx, action in enumerate(action_list):
                ...
                if is_active and action_type != "PASS":
                    # 检查是否有王或级牌（用于单张策略优先级调整�?                    has_king_or_level = has_king or has_level_card
                    ...
                    # 主动出牌时，单张策略调整
                        ...
                        if action_cards:
                            ...
                            if has_king_or_level:
                                # 有王/级牌保护，单张能回收，加�?                                score_adjustment += 30.0
                                strategy_reason = "主动出牌：有�?级牌保护，单张能回收"
                            # 级牌、大王等本身就有较高价值，加分
                            elif card_rank in ['2', 'B', 'R']:
                                score_adjustment += 25.0
                                strategy_reason = "主动出牌：级�?大王单张，有价�?
                            # 其他单张，根据牌力情况调整减分幅�?                            elif power >= 7:  # 牌力强时，单张减分幅度降�?                                score_adjustment -= 10.0
                                strategy_reason = "主动出牌：牌力强，单张减分幅度降�?
                            else:
                                score_adjustment -= 20.0
                                strategy_reason = "主动出牌：避免小单张"
                
                # 策略应用顺序（按优先级）�?                # 0. 保护队友机制（最高优先级：当队友已经压制对手时，不应该再次压制）
                # 0.5. 对手�?张时的紧急处理（极高优先级：禁止出小单张，必须用大牌压制或PASS�?                # 1. 组牌策略（优先：减少轮次、减少单牌）
                # 2. 牌力评估（用于后续策略判断）
                # 3. 单牌策略
                # 4. 炸弹策略
                # 5. 残局策略
                
                # 0. 保护队友机制：核心规�?- 队友出牌时必须PASS
                teammate_pos = game_state.get("teammate_pos", -1)
                cur_pos = game_state.get("cur_pos", -1)
                greater_pos = game_state.get("greater_pos", -1)
                greater_action = game_state.get("greater_action", [])
                
                # 0.5. **核心规则**：对手剩1张时的紧急处�?                # 获取所有对手的剩余牌数
                opponent_rest_cards_list = game_state.get("opponent_rest_cards_list", [27, 27, 27])
                my_pos = game_state.get("my_pos", 0)
                
                # **新增**：用王压制级�?小王的逻辑
                # 检查对手是否出了级牌或小王，如果有王应该压�?                if greater_action and len(greater_action) > 2:
                    greater_cards = greater_action[2] if isinstance(greater_action[2], list) else []
                    if greater_cards and len(greater_cards) == 1:
                        # 对手出的是单�?                        greater_card = greater_cards[0]
                        ...
                        if action_type == "Single" or action_type == "SINGLE":
                            if action_card_count == 1 and action_cards:
                                ...
                                # 如果对手出的是级牌（cur_rank），应该用小�?B)或大�?R)压制
                                if greater_rank == cur_rank:
                                    if action_rank == "B":  # 用小王压制级�?                                        score_adjustment += 120.0  # 大幅加分
                                        strategy_reason = f"核心规则：对手出级牌({cur_rank})，用小王(B)压制"
                                    elif action_rank == "R":  # 用大王压制级�?                                        score_adjustment += 150.0  # 大幅加分
                                        strategy_reason = f"核心规则：对手出级牌({cur_rank})，用大王(R)压制"
                                
                                # 如果对手出的是小�?B)，应该用大王(R)压制
                                elif greater_rank == "B":
                                    if action_rank == "R":  # 用大王压制小�?                                        score_adjustment += 130.0  # 大幅加分
                        ...
                        if action_type == "PASS":
                            ...
                            if greater_rank == cur_rank:
                                if has_small_king or has_big_king:
                                    score_adjustment -= 100.0  # 大幅减分，强制要求压�?                                    strategy_reason = f"核心规则：对手出级牌({cur_rank})，有王应该压制，不应PASS"
                            
                            # 如果对手出的是小�?B)，且有大王，应该压制
                            elif greater_rank == "B":
                                if has_big_king:
                                    score_adjustment -= 120.0  # 大幅减分，强制要求压�?                                    strategy_reason = "核心规则：对手出小王(B)，有大王应该压制，不应PASS"
                
                # 检查是否有对手只剩1张牌
                has_opponent_with_1_card = False
                opponent_with_1_card_pos = -1
                for i, rest_cards in enumerate(opponent_rest_cards_list):
                    # 对手位置：不是自己，也不是队�?                    if i != my_pos and i != (my_pos + 2) % 4:
                        if rest_cards == 1:
                ...
                if has_opponent_with_1_card and (action_type == "Single" or action_type == "SINGLE"):
                    if action_card_count == 1 and action_cards:
                        ...
                        if card_rank <= 9:  # 3-9 是小单张
                            score_adjustment -= 200.0  # 极大减分，强制禁�?                            strategy_reason = f"核心规则：对手（位置{opponent_with_1_card_pos}）剩1张，禁止出小单张（{card_rank_str}），必须用大牌压制或PASS"
                        elif card_rank >= 12:  # Q/K/A/2/B/R 是大�?                            score_adjustment += 100.0  # 大幅加分，鼓励用大牌压制
                            strategy_reason = f"核心规则：对手（位置{opponent_with_1_card_pos}）剩1张，使用大牌（{card_rank_str}）压�?
                    ...
                    if action_type == "Single" or action_type == "SINGLE":
                        if action_card_count == 1 and action_cards:
                            ...
                            if card_rank < 12:  # 小于Q
                                ...
                                pass
                    # 对于非单张动作，如果不是炸弹，也建议PASS让队友处�?                    elif action_type not in ["Bomb", "BOMB", "StraightFlush"]:
                        score_adjustment -= 50.0  # 减分，建议PASS让队友处�?                        strategy_reason = f"核心规则：对手（位置{opponent_with_1_card_pos}）剩1张，建议PASS让队友处理，避免浪费牌型"
                ...
                if cur_pos == teammate_pos and action_type != "PASS":
                    ...
                    strategy_reason = f"核心规则：队友（位置{cur_pos}）正在出牌，必须PASS，不能压制队�?
                
                # **核心规则2**：如果队友已经出牌（greater_pos == teammate_pos），必须PASS
                # 这是最重要的规则：队友出牌后，无论对手是否PASS，队友都不应该压制队�?                elif greater_pos == teammate_pos and action_type != "PASS":
                    score_adjustment -= 2000.0  # 极大减分，强制PASS（比规则1更严格）
                    strategy_reason = f"核心规则：队友（位置{greater_pos}）已经出牌，必须PASS，绝对不能压制队�?
                
                # **核心规则3**：如果队友已经压制了对手，且对手都PASS了，队友不应该再压制
                # 这个规则已经被规�?覆盖，但保留作为额外检�?                elif greater_pos == teammate_pos and action_type != "PASS":
                    # 检查是否与队友的牌型相同或相似
                    if greater_action and len(greater_action) > 0:
                        greater_type = greater_action[0] if isinstance(greater_action, list) else str(greater_action)
                        # 如果当前动作与队友的牌型相同，大幅减分（不应该重复压制）
                        if action_type == greater_type:
                            score_adjustment -= 500.0  # 大幅减分，避免重复压�?                            strategy_reason = f"保护队友：队友已用{greater_type}压制，不应重复压�?
                        # 如果当前动作是更大的牌型（如炸弹压制顺子），也减分（队友已经压制，不需要再压制�?                        elif action_type in ["Bomb", "BOMB", "StraightFlush"] and greater_type not in ["Bomb", "BOMB", "StraightFlush"]:
                            score_adjustment -= 400.0  # 减分，避免浪费炸�?                            strategy_reason = f"保护队友：队友已用{greater_type}压制，不应再用炸弹压�?
                ...
                # 通用规则：单张在开始阶段和中期，不要越过三级超�?                # 规则：在opening和mid阶段，单张出牌时，不应该超过当前牌型三级以上
                if action_type == "Single" or action_type == "SINGLE":
                    if game_phase in ["opening", "mid"]:
                        # 检查是否有当前牌型（greater_action�?                        greater_action = game_state.get("greater_action", [])
                        if greater_action and len(greater_action) > 0:
                            greater_type = greater_action[0] if isinstance(greater_action, list) else str(greater_action)
                            # 如果当前牌型也是单张，检查是否超过三�?                            if greater_type in ["Single", "SINGLE"]:
                                greater_cards = greater_action[2] if len(greater_action) > 2 and isinstance(greater_action[2], list) else []
                                if greater_cards and action_cards:
                                    ...
                                    if action_rank > greater_rank + 3:
                                        score_adjustment -= 40.0  # 大幅减分，避免超�?                                        strategy_reason = f"通用规则：单张在{game_phase}阶段，不应超过三级超打（当前{greater_rank_str}，出{action_rank_str}超过三级�?
                ...
                if action_type == "Single" or action_type == "SINGLE":
                    if action_cards:
                        ...
                        # 判断是否是天然单�?                        # 天然单张定义�?                        # 1. 整幅手牌只有唯一的一�?                        # 2. 级牌、小王、大王不要列入天然单�?                        is_natural_single = action_card in single_member and action_card[-1] != cur_rank and action_card[-1] != 'B' and action_card[-1] != 'R'
                        ...
                        # 判断是否是级�?                        is_rank_card = action_rank == cur_rank
                        
                        # 3.0.1 Normal模式：优先使用天然单�?                        # **关键修复**：先检查是否在炸弹/顺子中，如果是则大幅减分，避免拆炸弹
                        if is_in_bomb or is_in_straight:
                            # 在炸弹或顺子中，大幅减分（normal模式严格避免拆炸�?顺子�?                            score_adjustment -= 50.0  # �?15改为-50，大幅增加拆炸弹的惩�?                            strategy_reason = "单牌策略：normal模式，严格避免拆炸弹/顺子（拆炸弹代价高）"
                        elif is_natural_single and not is_rank_card:
                            score_adjustment += 40.0  # 天然单张大幅加分
                            strategy_reason = "单牌策略：normal模式，优先使用天然单�?
                        elif action_rank_val >= 15 and not is_rank_card:  # 牌�?=15�?、B、R）且不是级牌
                            score_adjustment += 35.0  # 大牌值单张加�?                            strategy_reason = "单牌策略：normal模式，使用大牌值单�?
                        elif is_rank_card:
                            # 级牌单张，根据情况判�?                            score_adjustment += 20.0  # 级牌单张中等加分
                            strategy_reason = "单牌策略：normal模式，级牌单�?
                        
                        # 3.0.2 Special模式：连续PASS过多时触发（pass_num >= 5 or my_pass_num >= 3�?                        if pass_num >= 5 or my_pass_num >= 3:
                            # special模式：从后往前遍历，不使用炸弹中的牌，不在顺子中
                            if is_in_bomb or is_in_straight:
                                # **关键修复**：special模式中，拆炸�?顺子的惩罚更严格
                                score_adjustment -= 80.0  # �?30改为-80，special模式严格禁止拆炸�?                                strategy_reason = f"单牌策略：special模式（连续PASS {pass_num}/{my_pass_num}次），严格禁止拆炸弹/顺子"
                            elif not is_rank_card:
                                score_adjustment += 50.0  # special模式大幅加分
                                strategy_reason = f"单牌策略：special模式（连续PASS {pass_num}/{my_pass_num}次），不使用炸弹/顺子中的�?
                        
                        # 3.0.3 残局时优先使用大单张（numofnext <= 4�?                        if numofnext <= 4:
                            # 残局：优先使�?>= max_val 且是天然单张的单�?                            if action_rank_val >= max_val and is_natural_single and not is_rank_card:
                                score_adjustment += 60.0  # 残局大单张大幅加�?                                strategy_reason = f"单牌策略：残局（下家剩{numofnext}张），优先使用大单张（天然单张，>=max_val�?
                            elif action_rank_val >= max_val and not is_in_bomb and not is_in_straight and not is_rank_card:
                                score_adjustment += 45.0  # 残局大单张（非天然但>=max_val）加�?                                strategy_reason = f"单牌策略：残局（下家剩{numofnext}张），使用大单张�?=max_val�?
                            elif action_rank_val >= max_val - 2 and not is_in_bomb and not is_in_straight and not is_rank_card:
                                score_adjustment += 35.0  # 残局接近大单张加�?                                strategy_reason = f"单牌策略：残局（下家剩{numofnext}张），使用接近大单张�?=max_val-2�?
                
                # 主动出牌策略（基于优先级和阈值）
                # 优先级：对子 > 三张 > 三带�?> 顺子 > 三连�?钢板 > 单张
                # cur = [9,10,9,8,10,10,2] 对应 [单张, 三连�?, 三连�?, 三带�? 顺子, 三带�?, 其他]
                if is_active and action_type != "PASS":
                    active_cur = [9, 10, 9, 8, 10, 10, 2]  # 固定阈�?                    
                    # 获取动作的牌值（用于判断是否小于阈值）
                    # 确保action_rank_val已定义，如果没有则使用默认�?                    if 'action_rank_val' not in locals():
                        action_rank_val = 0
                    ...
                    # 3. 三带二优先级第三�?60分，但需要满足阈值条件）
                        ...
                        if action_rank_val_for_active < active_cur[3]:  # cur[3] = 8
                            ...
                            score_adjustment += 40.0  # 不满足阈值，降低优先�?                            strategy_reason = "主动出牌策略：三带二（不满足阈值）"
                    ...
                    # 4. 顺子优先级第四（+50分，但需要满足阈值条件）
                        ...
                        if action_rank_val_for_active < active_cur[4]:  # cur[4] = 10
                            ...
                            score_adjustment += 30.0  # 不满足阈值，降低优先�?                            strategy_reason = "主动出牌策略：顺子（不满足阈值）"
                    ...
                    # 5. 三连�?钢板优先级第五（+40分）
                        ...
                        strategy_reason = "主动出牌策略：三连对/钢板优先级第�?
                    
                    # 6. 单张优先级最低（+20分，但需要满足阈值条件）
                    # **关键修复**：如果单张在炸弹中，即使满足阈值也不加分，避免拆炸�?                    elif action_type == "Single" or action_type == "SINGLE":
                        # 检查是否在炸弹中（需要重新获取is_in_bomb，因为可能不在单牌策略的if块内�?                        action_card_for_check = action_cards[0] if action_cards else ""
                        is_in_bomb_for_active = action_card_for_check in bomb_member if action_card_for_check else False
                        
                        if is_in_bomb_for_active:
                            # 拆炸弹的单张，即使满足阈值也不加分，反而减�?                            score_adjustment -= 30.0
                ...
                # 根据动作类型应用策略
                if action_type == "PASS":
                    # 单牌策略：不出小�?                    if single_sugg.get("action", "").startswith("不出"):
                        score_adjustment += 30.0
                    ...
                    for i, rest_cards in enumerate(opponent_rest_cards_list):
                        if i != my_pos and i != (my_pos + 2) % 4:
                            if rest_cards == 1:
                                score_adjustment += 80.0  # 大幅加分，鼓励PASS让队友处�?                                strategy_reason = f"核心规则：对手（位置{i}）剩1张，PASS让队友处理是优先选择"
                                break
                else:
                    # 非PASS动作
                    
                    # 单牌策略：全面应用所有建�?                    single_action = single_sugg.get("action", "")
                    single_reason = single_sugg.get("reason", "")
                    
                    # 1. 出单相关建议
                    if action_type == "Single" or action_type == "SINGLE":
                        # 核心规则：检查是否拆小对出单（禁止拆小对�?                        if action_card_count == 1 and action_cards:
                            card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                            ...
                            # 检查手牌中这个点数是否有对子（说明是拆对）
                                ...
                                # 这是拆对，检查是否是小对�?以下，即3�?�?                                if card_rank <= 4:  # 3, 4 是小�?                                    score_adjustment -= 50.0  # 大幅减分，禁止拆小对
                                    strategy_reason = f"核心规则：禁止拆小对（对{card_rank_str}）出单，应保留对�?
                            
                            # **新增**：优先使用天然单张（整副手牌只有唯一的一张）
                            # 检查是否是天然单张（不在炸弹、顺子、对子、三张中�?                            is_natural_single = (card_rank in rank_count and rank_count[card_rank] == 1 and 
                                                card_rank_str not in ['2', 'B', 'R'])  # 排除级牌、小王、大�?                            if is_natural_single:
                                score_adjustment += 50.0  # 天然单张大幅加分
                                strategy_reason = f"核心规则：优先使用天然单张（{card_rank_str}），避免拆三�?对子"
                        
                        if "出单" in single_action or "打一�? in single_action:
                            # **核心规则**：对手剩1张时的特殊处理（优先检查）
                            if "出第二小的单" in single_action:
                                # 对手�?张，主攻/中等牌力：出第二小的�?                                # 需要找到手牌中第二小的单张
                                if action_card_count == 1 and action_cards:
                                    # 获取所有单张的牌�?                                    single_card_values = []
                                    rank_map = {'3':3, '4':4, '5':5, '6':6, '7':7, '8':8, '9':9, 'T':10, 'J':11, 'Q':12, 'K':13, 'A':14, '2':15, 'B':16, 'R':17}
                                    for card in hand_cards:
                                        if len(card) >= 2:
                                            rank_str = card[1] if len(card) == 2 else card[1:2]
                                            if rank_str in rank_map:
                                                # 检查是否是单张（不在对子、三张中�?                                                if rank_count.get(rank_map[rank_str], 0) == 1:
                                                    single_card_values.append((rank_map[rank_str], card))
                                    
                                    if len(single_card_values) >= 2:
                                        # 排序，找到第二小�?                                        single_card_values.sort(key=lambda x: x[0])
                                        ...
                                        if action_rank == second_smallest_rank:
                                            ...
                                            score_adjustment -= 50.0  # 减分，不是第二小�?                                            strategy_reason = f"单牌策略：应出第二小的单，当前不�?
                                    else:
                                        # 单张不足2张，无法出第二小的，减分
                                        score_adjustment -= 30.0
                                        strategy_reason = "单牌策略：单张不足，无法出第二小的单"
                            elif "出第二大的单" in single_action:
                                # 对手�?张，助攻角色：出第二大的�?                                if action_card_count == 1 and action_cards:
                                    # 获取所有单张的牌�?                                    single_card_values = []
                                    rank_map = {'3':3, '4':4, '5':5, '6':6, '7':7, '8':8, '9':9, 'T':10, 'J':11, 'Q':12, 'K':13, 'A':14, '2':15, 'B':16, 'R':17}
                                    for card in hand_cards:
                                        if len(card) >= 2:
                                    ...
                                    if len(single_card_values) >= 2:
                                        # 排序，找到第二大�?                                        single_card_values.sort(key=lambda x: x[0], reverse=True)
                                        ...
                                        if action_rank == second_largest_rank:
                                            ...
                                            score_adjustment -= 50.0  # 减分，不是第二大�?                                            strategy_reason = f"单牌策略：应出第二大的单，当前不�?
                                    else:
                                        # 单张不足2张，无法出第二大的，减分
                                        score_adjustment -= 30.0
                                ...
                                if action_card_count == 1 and action_cards:
                                    ...
                                    if is_natural_single:
                                        score_adjustment += 50.0  # 天然单张更高�?                                        strategy_reason = "单牌策略：起始出天然单（有保护）"
                                    else:
                                        score_adjustment += 40.0
                                        strategy_reason = "单牌策略：起始出单（有保护）"
                            elif "起始出单" in single_action or "有保�? in single_reason:
                                # 有保护出单，大幅加分
                                score_adjustment += 40.0
                                strategy_reason = "单牌策略：有保护出单"
                            elif "出小单（有王回收�? in single_action:
                                # 有王回收时出小单，加�?                                if action_card_count == 1 and action_cards:
                                    card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank_str in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 45.0
                                        strategy_reason = "单牌策略：有王回收，先出小单再用王回收冲�?
                            elif "出单（一炸一顺两单）" in single_action:
                                # 一炸一顺两单，先出一单，加分
                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment += 35.0
                                    strategy_reason = "单牌策略：一炸一顺两单，先出一�?
                            elif "顺上家出�? in single_action:
                                # 顺上家出单，加分
                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment += 30.0
                                ...
                                # 单张多时出单，加�?                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment += 30.0
                                    strategy_reason = "单牌策略：单张多，先出单"
                            elif "出天然单（有�?级牌保护�? in single_action:
                                # 有王/级牌保护出天然单，大幅加�?                                if action_card_count == 1 and action_cards:
                                    card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    card_rank = rank_map.get(card_rank_str, 0)
                                    is_natural_single = (card_rank in rank_count and rank_count[card_rank] == 1 and 
                                                        card_rank_str not in ['2', 'B', 'R'])
                                    if is_natural_single:
                                        score_adjustment += 50.0
                                        strategy_reason = "单牌策略：有�?级牌保护，出天然�?
                                    else:
                                        score_adjustment += 40.0
                                        strategy_reason = "单牌策略：有�?级牌保护，出�?
                            elif "出单（有�?级牌保护�? in single_action:
                                # 有王/级牌保护出单，加�?                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment += 40.0
                                    strategy_reason = "单牌策略：有�?级牌保护，出�?
                                ...
                                if action_card_count == 1 and action_cards:
                                    ...
                                    if card_rank_str in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 40.0
                                        strategy_reason = "单牌策略：队友剩一张，放心出小�?
                            elif "继续出小�? in single_action:
                                # 对手不接小单，继续出小单压迫，加�?                                if action_card_count == 1 and action_cards:
                                    card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank_str in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 35.0
                                        strategy_reason = "单牌策略：对手不接小单，继续出小单压�?
                                ...
                                # 根据对手顺子历史出单防守，加�?                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment += 30.0
                                ...
                                if action_card_count == 1 and action_cards:
                                    ...
                                    if card_rank_str in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 30.0
                                        strategy_reason = "单牌策略：根据队友顺子历史，送小�?
                            elif "主动时不出小�? in single_action:
                                # 主动出牌时不出小单，对小单张减分
                                if action_card_count == 1 and action_cards:
                                    card_rank_str = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank_str in ['3', '4', '5', '6', '7', '8']:
                                        score_adjustment -= 30.0
                                        strategy_reason = "单牌策略：主动出牌时，牌力差，不出小�?
                                ...
                                # 助攻定位不出单，对单张动作减�?                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment -= 30.0
                                    strategy_reason = single_reason if single_reason else "单牌策略：助攻定位，不出�?
                            elif "让队友主�? in single_action or "主动不出�? in single_action:
                                # 让队友主导或主动不出单，对单张动作减�?                                if action_type == "Single" or action_type == "SINGLE":
                                    score_adjustment -= 25.0
                                    strategy_reason = single_reason if single_reason else "单牌策略：让队友主导，不出单"
                            elif "多炸保护" in single_reason:
                                # 多炸保护出单，加�?                                score_adjustment += 35.0
                                strategy_reason = "单牌策略：多炸保护出�?
                            elif "进贡大王" in single_reason:
                                # 进贡大王后出单，加分
                                score_adjustment += 30.0
                                strategy_reason = "单牌策略：进贡后出单"
                            elif "出高�? in single_action:
                                # 高单出牌，对高单张（Q、K、A�?、B、R）加�?                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['Q', 'K', 'A', '2', 'B', 'R']:
                                        score_adjustment += 45.0
                                        strategy_reason = "单牌策略：出高单（挡住下家中低单�?
                            elif "出中�? in single_action:
                                # 中单出牌，对中单张（J、T�?）加�?                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['J', 'T', '9']:
                                        score_adjustment += 35.0
                                        strategy_reason = "单牌策略：出中单（水闸试探）"
                            elif "出低�? in single_action:
                                # 低单出牌，对小单张（3-8）加�?                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['3', '4', '5', '6', '7', '8']:
                                        score_adjustment += 30.0
                                        strategy_reason = "单牌策略：出低单（传牌给对家�?
                            elif "控下家单" in single_action or "卡小" in single_reason:
                                # 控下家单，对小单张加�?                                if action_card_count == 1:
                                    # 检查是否是小单张（3-9�?                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 35.0
                                        strategy_reason = "单牌策略：控下家小单"
                            elif "送小�? in single_action or "让对�? in single_action:
                                # 让对家出单，对小单张加分
                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['3', '4', '5', '6', '7', '8', '9']:
                                        score_adjustment += 30.0
                                        strategy_reason = "单牌策略：送对家小�?
                            elif "顺子出中�? in single_action:
                                # 顺子出中间单，加�?                                score_adjustment += 25.0
                                strategy_reason = "单牌策略：顺子出中间"
                            elif "卡点出单" in single_action or "发级�? in single_reason:
                                # 卡点出单，对级牌加分
                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                ...
                                # 报双打单诱拆，加�?                                score_adjustment += 40.0
                                strategy_reason = "单牌策略：报双打单诱�?
                            elif "倒着�? in single_action:
                                # 出单倒着打，对高单张加分
                                if action_card_count == 1:
                                    card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                    if card_rank in ['Q', 'K', 'A', '2', 'B', 'R']:
                                ...
                                # 一般出单建�?                                score_adjustment += 30.0
                                strategy_reason = "单牌策略：出�?
                        elif "不出小单" in single_action or "不出�? in single_action or "不打�? in single_action:
                            # 不建议出单，对单张动作减�?                            score_adjustment -= 25.0
                            strategy_reason = "单牌策略：不出单"
                        elif "不出" in single_action and "小单" in single_action:
                            # 不出小单，对小单张减�?                            if action_card_count == 1:
                                card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                if card_rank in ['3', '4', '5', '6', '7', '8', '9']:
                                    score_adjustment -= 30.0
                                    strategy_reason = "单牌策略：不出小�?
                        elif "报单打非�? in single_reason:
                            # 报单不打单，对单张动作减�?                            score_adjustment -= 30.0
                            strategy_reason = "单牌策略：报单不打单"
                    
                    # 2. 出对相关建议
                    elif action_type == "Pair" or action_type == "PAIR":
                        if "出对" in single_action or "打两�? in single_action:
                            score_adjustment += 30.0
                            strategy_reason = "单牌策略：出�?
                        elif "拆大对出�? in single_action:
                            # 建议拆大对出单，对拆对动作减�?                            score_adjustment -= 20.0
                            strategy_reason = "单牌策略：不拆大�?
                        elif "无单拆大�? in single_action or "不出" in single_action:
                            # 不建议拆大对，对拆对动作减分
                            score_adjustment -= 15.0
                            strategy_reason = "单牌策略：不拆大�?
                    
                    # 3. 出三张相关建�?                    elif action_type == "Trips" or action_type == "TRIPS":
                        if "打三�? in single_action:
                            score_adjustment += 30.0
                            strategy_reason = "单牌策略：出三张"
                    
                    # 4. 三带二相关建�?                    elif action_type == "THREE_WITH_TWO":
                        if "打三带二" in single_action:
                            score_adjustment += 30.0
                            strategy_reason = "单牌策略：出三带�?
                ...
                # 4. 炸弹策略（基于牌力评估结果）- 关键策略，防止浪费炸�?                # 同花顺（StraightFlush）也是炸弹的一种，应该遵守炸弹使用规则
                if action_type == "BOMB" or "BOMB" in str(action_type).upper() or action_type == "Bomb" or action_type == "StraightFlush":
                    # **关键修复**：先检查对手剩余牌数，判断是否应该�?                    # 获取所有对手的剩余牌数，检查是否有对手�?�?                    has_opponent_with_4_cards = False
                    ...
                    # **核心规则**：炸不打四（火不打四�? 对手�?张时，强制禁止用炸弹/同花�?                    if has_opponent_with_4_cards:
                        score_adjustment -= 400.0  # 极大减分，强制禁�?                        strategy_reason = f"核心规则：炸不打四（对手位置{opponent_with_4_cards_pos}�?张），强制禁止用炸弹/同花�?
                    
                    # 检查炸弹策略建�?                    bomb_suggestions = bomb_sugg.get("suggestions", [])
                    should_bomb = False
                    should_not_bomb = False
                    
                    # 先检查是否有"不炸"的建议，优先级更�?                    for sugg in bomb_suggestions:
                        ...
                        if "不炸" in action_text:
                            ...
                            if "炸不打四" in action_text:
                                ...
                                if opponent_rest_cards == 4 or has_opponent_with_4_cards:
                                    score_adjustment -= 300.0  # 大幅减分，强制禁止（除非特殊情况�?                                    strategy_reason = f"炸弹策略：炸不打四（敌方�?张，强制禁止�?
                                else:
                                    score_adjustment -= 200.0
                            elif "牌型不明" in action_text or "走不�? in action_text:
                                score_adjustment -= 180.0  # 牌型不明或走不了，较大减�?                            elif "经济" in action_text or "隐蔽" in action_text or "配合" in action_text:
                                score_adjustment -= 100.0  # 经济、隐蔽、配合等原因，中等减�?                            else:
                                score_adjustment -= 200.0  # 其他不炸原因，大幅减�?                            break
                    
                    # 检查是否有"�?的建�?                    for sugg in bomb_suggestions:
                        action_text = sugg.get("action", "")
                        reason_text = sugg.get("reason", "")
                        if "�? in action_text and "不炸" not in action_text:
                            should_bomb = True
                            strategy_reason = reason_text
                            # 根据不同�?�?原因，给予不同的加分
                            if "�?" in action_text or "判敌4+3" in action_text:
                                score_adjustment += 60.0  # �?要提前炸，大幅加�?                            elif "管压" in action_text or "压制" in action_text:
                                score_adjustment += 50.0  # 管压对手，加�?                            elif "改牌�? in action_text or "控牌" in action_text:
                                score_adjustment += 40.0  # 改牌路或控牌，加�?                            else:
                                score_adjustment += 30.0  # 其他炸的原因，中等加�?                            break
                ...
                if action_type == "BOMB" or "BOMB" in str(action_type).upper() or action_type == "Bomb" or action_type == "StraightFlush":
                    ...
                    # 4.2 检查炸弹策略建�?                    bomb_suggestions = bomb_sugg.get("suggestions", [])
                    should_not_bomb = False
                    for sugg in bomb_suggestions:
                        if "�? in sugg.get("action", "") and "不炸" not in sugg.get("action", ""):
                            score_adjustment += 40.0
                    ...
                    # 4.3 如果没有明确�?应该�?建议，检查是否不应该�?                    if not should_not_bomb:
                        # 检查当前牌型：单张、对子、三张等小牌型不值得�?                        greater_action = game_state.get("greater_action", [])
                        if greater_action and len(greater_action) > 0:
                            greater_type = greater_action[0] if isinstance(greater_action, list) else str(greater_action)
                            greater_cards = greater_action[2] if len(greater_action) > 2 and isinstance(greater_action[2], list) else []
                            greater_card_count = len(greater_cards)
                            
                            # 单张、对子、三张不值得炸（除非残局�?                            if greater_type in ["Single", "SINGLE", "Pair", "PAIR", "Trips", "TRIPS"]:
                                if opponent_rest_cards > 10:  # 非残局
                                    should_not_bomb = True
                                    strategy_reason = f"炸弹策略：{greater_type}不值得炸（非残局�?
                    
                    # 4.4 应用炸弹选择优先级（优先使用不灵活的炸弹�?                    # 收集所有炸弹动�?                    bomb_actions = []
                    ...
                    if len(bomb_actions) > 1:
                        ...
                        # 获取牌值映�?                        rank_map = {'3':3, '4':4, '5':5, '6':6, '7':7, '8':8, '9':9, 'T':10, 'J':11, 'Q':12, 'K':13, 'A':14, '2':15, 'B':16, 'R':17}
                        card_val = rank_map.copy()
                        card_val[cur_rank] = 15
                        
                        # 选择最佳炸�?                        best_bomb_idx, bomb_reason = select_bomb_priority(
                            bomb_actions, hand_cards, sorted_cards_dict, bomb_info_dict, rank_card, card_val
                        )
                        
                        # 如果当前动作是最佳炸弹，加分；否则减�?                        if best_bomb_idx == idx:
                ...
                if power >= 8:
                    ...
                    # 弱牌，PASS或保守动作加�?                    if action_type == "PASS" or action_card_count <= 2:
                        score_adjustment += 20.0
                ...
                # 获取动作的牌点（用于策略判断�?                action_rank_str = ""
                ...
                # 判断是否是队友出�?                is_teammate_action = (greater_pos == teammate_pos)
                ...
                if action_type == "Pair" or action_type == "PAIR":
                    pair_sugg = pair_strategy(
                        ...
                        can_form_three_pair=False,  # TODO: 需要从组牌策略中获�?                        can_form_straight=False,  # TODO: 需要从组牌策略中获�?                        is_first_place_finished=is_first_place_finished
                    )
                    ...
                    if "情况不明对子先行" in pair_action:
                        ...
                        score_adjustment += 60.0
                        strategy_reason = f"对子策略：{pair_reason}"
                    elif "�?0出对�? in pair_action:
                        score_adjustment += 50.0
                        strategy_reason = f"对子策略：{pair_reason}"
                    elif "顺下中对" in pair_action or "送对�? in pair_action:
                        score_adjustment += 40.0
                        strategy_reason = f"对子策略：{pair_reason}"
                    elif "顶大对子" in pair_action or "封对手对�? in pair_action:
                        score_adjustment += 45.0
                        strategy_reason = f"对子策略：{pair_reason}"
                    elif "让对�? in pair_action:
                        if action_type == "PASS":
                            score_adjustment += 30.0
                            strategy_reason = f"对子策略：{pair_reason}"
                        else:
                ...
                # 4.5.2 顺子策略
                    ...
                    straight_sugg = straight_strategy(
                        ...
                        has_bomb=has_bomb,
                        single_card_count=single_card_count,
                        can_form_straight=False,  # TODO: 需要从组牌策略中获�?                        is_first_place_finished=is_first_place_finished,
                        has_king=has_king
                    )
                    ...
                    if "牌弱先出�? in straight_action:
                        ...
                        if is_teammate_action and action_type == "PASS":
                            ...
                            score_adjustment -= 30.0
                            strategy_reason = f"顺子策略：{straight_reason}"
                    elif "顺子管到�? in straight_action:
                        score_adjustment += 55.0
                        strategy_reason = f"顺子策略：{straight_reason}"
                    elif "七张八张，打顺打�? in straight_action:
                        score_adjustment += 50.0
                        strategy_reason = f"顺子策略：{straight_reason}"
                    elif "九张十张，不出顺�? in straight_action:
                        score_adjustment -= 40.0
                        strategy_reason = f"顺子策略：{straight_reason}"
                    elif "组顺生两�? in straight_action:
                        score_adjustment -= 60.0
                        strategy_reason = f"顺子策略：{straight_reason}"
                ...
                # 4.5.3 三张策略
                    ...
                    elif "三张小，对子�? in trips_action:
                        score_adjustment += 45.0
                        strategy_reason = f"三张策略：{trips_reason}"
                    elif "搅局" in trips_action:
                        score_adjustment += 40.0
                        strategy_reason = f"三张策略：{trips_reason}"
                    elif "送队友三�? in trips_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三张策略：{trips_reason}"
                    elif "顶大牌拦�? in trips_action:
                        score_adjustment += 55.0
                        strategy_reason = f"三张策略：{trips_reason}"
                
                # 4.5.4 三带二策�?                elif action_type == "ThreeWithTwo" or action_type == "THREE_WITH_TWO":
                    ...
                    # 根据三带二策略建议调整评�?                    if "有打有收" in three_with_two_action:
                        score_adjustment += 55.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "相生相克反打" in three_with_two_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "先出大三带二�? in three_with_two_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "反手出三带二" in three_with_two_action:
                        score_adjustment += 45.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "对手七张八张，出�? in three_with_two_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "对手剩余5�?�?0张，一般不出夯" in three_with_two_action:
                        score_adjustment -= 40.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "小夯尽快�? in three_with_two_action or "尽快要送夯" in three_with_two_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三带二策略：{three_with_two_reason}"
                    elif "不接队友�? in three_with_two_action:
                        if action_type == "PASS":
                            score_adjustment += 35.0
                            strategy_reason = f"三带二策略：{three_with_two_reason}"
                        else:
                ...
                # 4.5.5 钢板策略
                    ...
                    if "牌力中等，小钢板先出" in two_trips_action:
                        ...
                        score_adjustment -= 40.0
                        strategy_reason = f"钢板策略：{two_trips_reason}"
                    elif "牌力强，小钢板后�? in two_trips_action:
                        score_adjustment += 45.0
                        strategy_reason = f"钢板策略：{two_trips_reason}"
                    elif "拆分小钢板，传牌给队�? in two_trips_action:
                        score_adjustment += 50.0
                        strategy_reason = f"钢板策略：{two_trips_reason}"
                ...
                # 4.5.6 三连对策�?                elif action_type == "ThreePair" or action_type == "THREE_PAIR":
                    three_pair_ranks = []
                    for a in action_list:
                    ...
                    # 根据三连对策略建议调整评�?                    if "首引一般不轻易出木�? in three_pair_action:
                        ...
                        if is_teammate_action:
                            if action_type == "PASS":
                                ...
                                score_adjustment -= 30.0
                                strategy_reason = f"三连对策略：{three_pair_reason}"
                    elif "木板直接封到�? in three_pair_action:
                        score_adjustment += 55.0
                        strategy_reason = f"三连对策略：{three_pair_reason}"
                    elif "送队友木�? in three_pair_action:
                        score_adjustment += 50.0
                        strategy_reason = f"三连对策略：{three_pair_reason}"
                ...
                # 5. 残局策略（最后应用：根据残局情况，集成单张技巧残局规则�?                endgame_action = endgame_sugg.get("action", "")
                endgame_reason = endgame_sugg.get("reason", "")
                
                # **关键修复**：先检查对手剩余牌数，判断是否有对手剩7�?                # 获取所有对手的剩余牌数，检查是否有对手�?�?                has_opponent_with_7_cards = False
                ...
                if has_opponent_with_7_cards:
                    if action_type == "Straight" or action_type == "STRAIGHT":
                        ...
                        score_adjustment -= 100.0  # 大幅减分，强制要求出�?                        strategy_reason = f"核心规则：对手（位置{opponent_with_7_cards_pos}）剩7张，不应PASS，应出顺/三带�?�?
                    elif action_type == "Single" or action_type == "SINGLE":
                        # 对手�?张，出单张也可以，但不如出顺/三带�?�?                        score_adjustment += 30.0
                        strategy_reason = f"核心规则：对手（位置{opponent_with_7_cards_pos}）剩7张，出单可接受但不如出顺/三带�?�?
                
                # 5.0 优先判断：能否一手出完（one_hand函数逻辑�?                if "一手出�? in endgame_action and "one_hand_index" in endgame_sugg:
                    ...
                    if one_hand_index == idx:
                        ...
                        # 有其他动作可以一手出完，当前动作减分
                        score_adjustment -= 50.0
                        strategy_reason = f"残局策略：有其他动作可以一手出完，当前动作不是最�?
                ...
                if game_phase == "endgame" or opponent_rest_cards <= 10:
                    ...
                    # 2. 报双.须打单诱其拆
                        ...
                        if action_type == "Single" or action_type == "SINGLE":
                            score_adjustment += 40.0
                            strategy_reason = "残局策略：报双打单诱�?
                    
                    # 3. 报单.只能打非单牌�?                    elif "报单打非�? in endgame_reason or "不打�? in endgame_action:
                        if action_type == "Single" or action_type == "SINGLE":
                            score_adjustment -= 30.0
                    ...
                    # 4. 出单倒着打：从大往小打
                    elif "倒着�? in endgame_action or "从大往�? in endgame_reason:
                        if action_type == "Single" or action_type == "SINGLE":
                            if action_card_count == 1:
                                card_rank = action_cards[0][1] if len(action_cards[0]) >= 2 else ""
                                # 高单张加分，小单张减�?                                if card_rank in ['Q', 'K', 'A', '2', 'B', 'R']:
                                    score_adjustment += 35.0
                    ...
                    # 6. 残局其他建议（根据对手剩余牌数）
                        ...
                        if action_type != "PASS":
                            score_adjustment -= 30.0
                            strategy_reason = "残局策略：火不打四，观察或放给对�?
                    elif "放过给对�? in endgame_action:
                        # 放过给对家，PASS加分
                        if action_type == "PASS":
                            score_adjustment += 40.0
                            strategy_reason = "残局策略：不能压，放给对家处�?
                        else:
                            score_adjustment -= 20.0
                            strategy_reason = "残局策略：应放过给对�?
                    elif "出两�? in endgame_action or "打两张（对子�? in endgame_action:
                        # 残局出对，对对子动作加分
                        if action_type == "Pair" or action_type == "PAIR":
                            score_adjustment += 35.0
                            strategy_reason = "残局策略：出对试�?
                        elif action_type == "Single" or action_type == "SINGLE":
                            score_adjustment -= 20.0
                            strategy_reason = "残局策略：应出对，不应出�?
                    elif "打一�? in endgame_action or "打一张（单张�? in endgame_action:
                        # 残局出单，对单张动作加分
                        if action_type == "Single" or action_type == "SINGLE":
                            score_adjustment += 35.0
                            strategy_reason = "残局策略：出�?
                        elif action_type != "PASS":
                            score_adjustment -= 20.0
                        ...
                        if action_type == "Straight" or action_type == "STRAIGHT":
                            ...
                            score_adjustment += 40.0
                            strategy_reason = "残局策略：出三带�?
                        elif action_type == "Bomb" or action_type == "BOMB":
                            # 残局7张时可以考虑�?                            if opponent_rest_cards == 7:
                                score_adjustment += 30.0
                                strategy_reason = "残局策略：剩7可考虑�?
                ...
                # 为所有动作生成策略评分（即使没有特殊调整�?                # 大幅提高基础评分，让策略建议更有竞争力（不乘以权重，直接使用高分�?                base_score = 200.0  # 大幅提高基础规则评分�?00（不乘以权重�?                
                # 主动出牌时，进一步提高基础评分，确保主动出牌优�?                if is_active and action_type != "PASS":
                    base_score += 50.0  # 主动出牌额外加分（提高）
                
                final_score = base_score + score_adjustment + base_strategy_score
                # 确保评分不为负数，至少保持基础评分
                if final_score < 0:
                    final_score = base_score * 0.5  # 即使建议不炸，也保持一定评�?                
    ...
    def _hybrid_decision(self, data: dict, action_list: list) -> int:
        ...
        融合多种决策源：
        1. RL决策（如果可用）
        2. 知识库增强决策（HybridDecisionEngineV6�?        3. 规则引擎决策（新增策略：牌力评估、单牌策略、炸弹策略、残局策略�?        
        Args:
            data: 游戏状态消�?            action_list: 可用动作列表
        ...
        try:
            # 使用V6引擎生成候选（已经包含知识库增强）
            base_candidates = self.decision_engine._generate_candidates(data)
            if base_candidates:
                # 增强候选（知识库已应用�?                enhanced_candidates = self.decision_engine._enhance_candidates(base_candidates, data)
                for idx, score, layer in enhanced_candidates:
                    # 应用知识库权�?                    weighted_score = score * self.knowledge_weight
        ...
        if self.rl_available and self.rl_engine:
            try:
                # 注入手牌信息
                # 优先使用服务器发送的最新handCards，如果没有则使用自己维护�?                server_hand_cards = data.get("handCards", [])
                if server_hand_cards:
                    # 服务器发送了最新手牌，使用服务器的（更准确�?                    hand_cards_for_rl = server_hand_cards
                    ...
                    # 服务器没有发送，使用自己维护�?                    hand_cards_for_rl = self.hand_cards
                ...
                if rl_action is not None and 0 <= rl_action < len(action_list):
                    # RL决策评分（使用权重）
                    rl_score = 150.0 * self.rl_weight  # 提高基础RL评分�?50，让RL决策更有竞争�?                    candidates.append((rl_action, rl_score, "RL"))
        ...
        # 3. 应用新增策略（牌力评估、单牌策略、炸弹策略、残局策略�?        try:
        ...
        for idx, score, source in candidates:
            if idx not in merged_candidates:
                ...
                if score > existing_score:
                    merged_candidates[idx] = (score, source)
                # 如果是策略建议，优先保留（策略评分通常更高�?                elif "Strategy" in source and "Strategy" not in existing_source:
                    merged_candidates[idx] = (score, source)
    ...
    def _hybrid_decision_with_details(self, data: dict, action_list: list) -> tuple:
        """
        V6增强：智能混合决策（返回详细信息用于记录�?
        Returns:
            tuple: (action_index, score, layer, candidates)
        """
        # 复用现有�?_hybrid_decision 逻辑，但修改返回格式
        import time
        ...
        # 记录决策统计到决策引�?        duration = time.time() - decision_start_time
    ...
    def handle_notification(self, data: dict):
        ...
        if stage == "gameResult":
            ...
            self.logger.info("=" * 60)
            self.logger.info("GAME RESULT (V6)")
            self.logger.info("=" * 60)
        ...
        # 处理所有其他notify消息（包括没有stage或stage不是预期值的�?        if not stage or stage not in ["beginning", "play"]:
            ...
            if "curAction" in data or "curPos" in data:
                ...
                if cur_action and len(cur_action) > 0:
                    # 显示当前动作和最大动作信息（类似client3/4�?                    print(f"当前动作为{cur_pos}�?{cur_action}�?最大动作为{greater_pos}�?{greater_action}")

                    # 显示剩余牌数信息（类似client3/4�?                    public_info = data.get("publicInfo", [])
                    if public_info:
                        remaining_info = []
                        for i, info in enumerate(public_info):
                            if isinstance(info, dict) and "rest" in info:
                                ...
                                remaining_info.append("27")  # 默认�?                        if remaining_info:
                    ...
                    # 格式化出牌信�?                    if cur_action[0] != "PASS":
                        ...
                        # 也显示PASS信息，保持与client3/4的一致�?                        action_str = f"{cur_pos}号位打出{cur_action}"
                        greater_str = f"最大动作为{greater_pos}号位打出的{greater_action}" if greater_action else ""
                        print(f"{action_str}�?{greater_str}")
                        self.logger.info(f"{action_str}�?{greater_str}")
        ...
        if is_beginning:
            ...
            # 调试：检查手牌中是否有重复卡�?            from collections import Counter
            ...
            # 更新实际位置（服务器根据连接顺序分配�?            # �?个连�?�?位置0，第2个连�?�?位置1，第3个连�?�?位置2，第4个连�?�?位置3
            if my_pos != self.player_id:
                print(f"[yf2_v6] 位置更新：期望位置{self.player_id} �?实际位置{my_pos}号位（服务器根据连接顺序分配�?)
                self.logger.info(f"Position updated: {self.player_id} -> {my_pos} (服务器根据连接顺序分�?")
                self.player_id = my_pos
            else:
                # 位置与期望一致，也显示一�?                print(f"[yf2_v6] 我在{my_pos}号位（与期望位置一致）")
            
            # 打印手牌信息
            print(f"游戏开�? 我是{my_pos}号位，手牌：{hand_cards}")
            self.logger.info(f"游戏开�? 我是{my_pos}号位，手牌：{hand_cards}")
            
            # 打印等级信息（用于调试）
            # 尝试多种可能的字段名组合（按照平台实际发送的字段优先�?            self_rank = (data.get("selfRank") or data.get("self_rank") or data.get("myRank") or
            ...
            # **新增**：尝试平台实际可能使用的字段名（基于client客户端的显示�?            # 如果标准字段都找不到，尝试从服务器消息的实际结构中查�?            if self_rank == "?" or oppo_rank == "?" or cur_rank == "?":
                # 打印所有可能的等级相关字段，帮助调�?                rank_fields = []
                ...
                if rank_fields:
                    print(f"[等级字段调试] 发现可能的等级相关字�? {rank_fields}")
                else:
                    print(f"[等级字段调试] 未发现任何等级相关字�?)

                # 尝试从游戏状态或上下文信息中获取
                # 有时候等级信息可能在其他地方

            # 增强兼容性：尝试更多可能的等级信息字�?            # **修复**：基于client客户端显示字母等级，尝试从服务器消息中直接查�?            rank_found = False
            ...
            if self_rank == "?":
                ...
                for key in ["rank", "level", "grade", "playerRank"]:
                    if key in data and data[key] is not None:
                        ...
                        rank_found = True
                        print(f"[等级修复] 从字�?'{key}' 获取self_rank: {self_rank}")
                        break

            if oppo_rank == "?":
                # 对于oppo_rank，尝试team相关的字�?                for key in ["opponent_rank", "teamRank", "enemyRank"]:
                    if key in data and data[key] is not None:
                        oppo_rank = str(data[key])
                        rank_found = True
                        print(f"[等级修复] 从字�?'{key}' 获取oppo_rank: {oppo_rank}")
                        break

            if cur_rank == "?":
                # 尝试更多可能的当前等级字�?                for key in ["currentRank", "gameRank", "activeRank"]:
                    if key in data and data[key] is not None:
                        cur_rank = str(data[key])
                        rank_found = True
                        print(f"[等级修复] 从字�?'{key}' 获取cur_rank: {cur_rank}")
                        break

            # 如果还是找不到，尝试从消息的嵌套结构中查�?            if not rank_found and (self_rank == "?" or oppo_rank == "?" or cur_rank == "?"):
                print(f"[等级修复] 尝试从消息结构中查找等级信息...")

                # 检查是否有嵌套的等级信�?                for key, value in data.items():
            ...
            if cur_rank == "?":
                ...
                cur_rank = "2"
                print(f"[Warning] 无法获取当前等级，使用默认�? {cur_rank}")
            ...
            # 如果还是 "?"，进行更详细的调�?            if self_rank == "?" or oppo_rank == "?" or cur_rank == "?":
                print(f"[Debug] 等级信息获取失败，开始详细调�?..")

                # 打印所有可能的等级相关�?                available_keys = [k for k in data.keys() if any(term in k.lower() for term in ['rank', 'level', 'grade', 'player', 'team', 'enemy', 'current', 'game'])]
                if available_keys:
            ...
            # 尝试获取所有玩家的手牌信息
            all_players_hands = {}
            all_players_hands[my_pos] = hand_cards  # 自己的手�?            
            # 从publicInfo中获取其他玩家的剩余牌数（如果有�?            public_info = data.get("publicInfo", [])
            if public_info:
                for i, player_info in enumerate(public_info):
                    if isinstance(player_info, dict) and "rest" in player_info:
                        pass
            
            # 从restCards中获取其他玩家的手牌（如果有�?            rest_cards = data.get("restCards", [])
            if rest_cards:
                for rest_info in rest_cards:
                    if isinstance(rest_info, list) and len(rest_info) >= 2:
                        ...
                        if pos != my_pos:
                            ...
                            self.logger.info(f"记录{pos}号位手牌: {len(cards)}�?)
            
            # 确保my_pos也是整数�?            if isinstance(my_pos, str):
                try:
                    my_pos = int(my_pos)
                except:
                    pass
            ...
            # 记录每个玩家的出牌信息（用于回放�?            cur_pos = data.get("curPos", -1)
            cur_action = data.get("curAction", [])
            greater_pos = data.get("greaterPos", -1)
            greater_action = data.get("greaterAction", [])
                ...
                if rest_cards and self.game_recorder:
                    ...
                    if all_hands:
                        ...
                        if hasattr(self.game_recorder, 'current_game') and self.game_recorder.current_game:
                            ...
                            self.logger.info(f"从第一个play消息中获取所有玩家手�? {list(all_hands.keys())}")
                self._first_play_processed = True
            
            # 更新连续PASS计数（用于special模式�?            if cur_action and len(cur_action) > 0:
            ...
            # 格式化出牌信�?            if cur_action and len(cur_action) > 0 and cur_action[0] != "PASS":
                action_str = f"{cur_pos}号位打出{cur_action}"
                greater_str = f"最大动作为{greater_pos}号位打出的{greater_action}" if greater_action else ""
                print(f"{action_str}�?{greater_str}")
                self.logger.info(f"{action_str}�?{greater_str}")
    ...
    def _handle_tribute_action(self, data: dict):
        ...
        # 显示可选的进贡�?        action_list = data.get("actionList", {})
    ...
    def _handle_back_action(self, data: dict):
        ...
        # 显示可选的还贡�?        action_list = data.get("actionList", {})
        if "back" in action_list:
            ...
            # 保存战绩�?game_scores.json（用于GUI显示�?            self._save_game_scores(victory_num)
    ...
    def _save_game_scores(self, victory_num: list):
        """
        保存战绩�?game_scores.json（用于GUI显示�?        
        注意：服务器根据连接顺序来确定座�?        - �?个连�?�?位置0
        - �?个连�?�?位置1
        - �?个连�?�?位置2
        - �?个连�?�?位置3
        
        组队规则�?        - �?个连�?位置0)和第3个连�?位置2)自动为一�?        - �?个连�?位置1)和第4个连�?位置3)自动为一�?        
        标准启动顺序（确保yf1_V6和yf2_V6在同一队）�?        1. yf1_V6.py �?位置0 (Team A)
        2. run_lalala_client3.py �?位置1 (Team B)
        3. yf2_v6.py �?位置2 (Team A)
        ...
        # 判断获胜队伍
        # 服务器根据连接顺序确定座位：
        # - 如果严格按照标准启动顺序，yf1_V6在位�?，yf2_V6在位�?，它们是队友
        # - 但如果启动顺序不一致，可能不在同一�?        # 解决方案：根据当前client的player_name判断
        # - 如果当前client是yf1_V6或yf2_V6，那么我的位置就是Team A的一部分
        # - 我的队友位置（根据掼蛋规则：0�?是队友，1�?是队友）也是Team A的一部分
        # - 其他两个位置是Team B
        if len(victory_num) >= 4:
            my_pos = self.player_id
            # 根据掼蛋规则�?�?是队友，1�?是队�?            teammate_pos = (my_pos + 2) % 4
            opponent1_pos = (my_pos + 1) % 4
            opponent2_pos = (my_pos + 3) % 4
            
            # 判断当前client是否是我们的AI（yf1_V6或yf2_V6�?            is_yf_ai = self.user_info in ["yf1_V6", "yf2_v6"]
            ...
            if is_yf_ai:
                ...
                # Team A: 对手位置（假设对手是yf1_V6和yf2_V6�?                team_a_score = victory_num[opponent1_pos] + victory_num[opponent2_pos]
                # Team B: 当前位置 + 队友位置
...
async def main():
    ...
    client = yf2_v6_Client(player_id=2)
