# -*- coding: utf-8 -*-
"""
yf2_m2 - YiFei AI M2 Client (Player 2)
...
log_filename = log_dir / f"yf2_m2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class YF2_M2_Client:
    """
    YiFei AI M2 Client - Player 2
    M2版本：重构硬编码规则引擎
    """

    def __init__(self, player_id=2, use_local_websocket=True):
        self.player_id = player_id
        self.user_info = "yf2_m2"
        self.logger = logging.getLogger(f"yf2_m2")
        ...
        self.game_recorder = GameRecorder(player_id, "yf2_m2")
        ...
        self.logger.info("yf2_m2 initialized (Player {})".format(player_id))
    ...
    async def connect(self):
        try:
            self.logger.info("[yf2_m2] 等待连接延迟 {} 秒，确保第三个位置...".format(DELAY_BEFORE_CONNECT))
            time.sleep(DELAY_BEFORE_CONNECT)
            self.logger.info("[yf2_m2] 开始连接 ws://127.0.0.1:23456/game/yf2_m2")
            ...
            print("[yf2_m2] 连接成功！期望位置：{}号位（实际位置将在游戏开始时由服务器分配）".format(self.player_id))
    ...
    def _determine_game_result(self, order: list) -> str:
    ...
    def _handle_game_over(self, data: dict):
        ...
        if order and isinstance(order, list) and len(order) >= 4:
            ...
            self.logger.info("本局结果: %s (yf1_m2 负责写入 score 文件)", result_label)
...
async def main():
    client = YF2_M2_Client(player_id=2)
