...
class YF1_M1_Client:
    ...
    async def handle_action_request(self, data: dict):
        ...
        try:
            ...
            # 入口规范化：保证决策引擎拿到的手牌与 actionList 为统一字符串格式
    ...
    def _handle_game_start(self, data: dict):
        """处理游戏开始通知（兼容多种格式）"""
        # 兼容多种格式：handCards字段或playerPosition字段，入口统一规范为字符串列表
        ...
        self.logger.info(
            "[座位排查] 来源=yf1_m1._handle_game_start, 原始myPos=%s, 原始playerPosition=%s, 同步后player_id=%s",
        ...
        # ⚠️ 重要：尝试从publicInfo获取所有玩家的手牌信息，并规范为字符串列表
        all_players_hands = {}
        public_info = data.get("publicInfo", [])
        if public_info and isinstance(public_info, list):
            for i, info in enumerate(public_info):
    ...
    def _extract_result_from_notification(self, data: dict) -> dict:
        ...
        if not result.get("victoryNum"):
            # 兼容部分服务器将 victoryNum/draws 放在顶层
    ...
    def _update_latest_record_with_result(self, result: dict):
        ...
        try:
            ...
            yf1_records = sorted(
                game_records_dir.glob("*yf1_m1*.json"),
            ...
            if latest_record is None or record_data is None:
                self.logger.warning("未找到可解析的 yf1_m1 记录文件，无法补写 gameResult")
                return
