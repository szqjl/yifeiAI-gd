...
# 日志文件名：yf1_v4_YYYYMMDD_HHMMSS.log
log_filename = log_dir / f"yf1_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class YF1_V4_Client:
    """
    YiFei AI V4 Client - Player 0
    Uses HybridDecisionEngineV4 for robust decision making
    """
    
    def __init__(self, player_id=0):
        self.player_id = player_id
        self.user_info = "yf1_v4"
        self.websocket = None
        self.logger = logging.getLogger(f"yf1_v4")
        ...
        self.game_recorder = GameRecorder(player_id, "yf1_v4")
        
        self.logger.info(f"✓ yf1_v4 initialized (Player {player_id})")
    ...
    async def handle_action_request(self, data: dict):
        ...
        try:
            ...
            if self.use_rl:
                data["handCards"] = self.hand_cards if self.hand_cards else data.get("handCards", [])
    ...
    def handle_notification(self, data: dict):
        ...
        if stage == "beginning":
            ...
            self.logger.info(
                "[座位排查] 来源=yf1_v4.beginning, 原始myPos=%s, 原始playerPosition=%s, 同步后player_id=%s",
...
async def main():
    """Main entry point"""
    client = YF1_V4_Client(player_id=0)
