...
def check_training_complete(log_file):
    ...
    try:
        with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if 'Training completed' in last_lines or 'Epoch 150/150' in last_lines:
                return True
...
def evaluate_model():
    ...
    try:
        ...
        # 保存评估结果
        result_file = f"training_logs/solution_f_evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(result_file, 'w', encoding='utf-8') as f:
            f.write("方案F评估结果（state_vec编码修复后）\n")
            f.write("="*60 + "\n")
            f.write(f"评估时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("="*60 + "\n\n")
            f.write("修复内容：\n")
            f.write("1. 历史动作编码（164-511维）：88.6%样本有数据\n")
            f.write("2. 策略标志计算（152-154维）：88.0%样本有数据\n")
            f.write("3. 策略效果利用（163维）：已确认正确\n")
            f.write("="*60 + "\n\n")
...
def main():
    print("="*60)
    print("方案F训练监控脚本（state_vec编码修复后）")
    print("="*60)
