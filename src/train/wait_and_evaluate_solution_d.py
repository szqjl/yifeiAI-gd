...
def get_latest_training_log():
    """获取最新的训练日志文件"""
    log_files = glob.glob("training_logs/solution_d_training_*.log")
    if not log_files:
        return None
...
def wait_for_training_complete(log_file=None, max_wait_minutes=180):
    """等待训练完成"""
    if log_file is None:
        log_file = get_latest_training_log()
    
    if log_file is None:
        print("[ERROR] 找不到训练日志文件")
        return False
...
if __name__ == "__main__":
    ...
    if log_file is None:
        print("[ERROR] 找不到训练日志文件，请确认训练已启动")
        sys.exit(1)
    ...
    if wait_for_training_complete(log_file, max_wait_minutes=180):
        ...
        # 评估结果
        eval_result = evaluate_solution_d()
        
        if eval_result:
            # 保存评估结果
            result_file = f"training_logs/solution_d_evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
